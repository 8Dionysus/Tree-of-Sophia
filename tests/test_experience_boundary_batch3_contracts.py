...
ENUM_ESCAPE_VALUE = "__batch3_not_allowed__"
...
class ExperienceBatch3ContractTests(unittest.TestCase):
    ...
    def test_experience_batch3_examples_match_schemas(self) -> None:
        missing_pairs: list[str] = []
        for stem in BATCH3_STEMS:
            schema_path, example_path = contract_paths(stem)
        ...
        self.assertFalse(missing_pairs, "missing batch3 contract pair(s): " + ", ".join(missing_pairs))

        self.assertTrue(BATCH3_STEMS)
        for stem in BATCH3_STEMS:
    ...
    def test_experience_batch3_schemas_reject_escape_hatches(self) -> None:
        self.assertTrue(BATCH3_STEMS)
        for stem in BATCH3_STEMS:
            with self.subTest(stem=stem):
                schema, example = load_contract(stem)
    ...
    def test_experience_batch3_schemas_reject_guardrail_boolean_inversions(self) -> None:
        for stem in BATCH3_STEMS:
            schema, example = load_contract(stem)
            payload = example.get("payload")
            if not isinstance(payload, dict):
    ...
    def test_experience_batch3_schemas_reject_invalid_numeric_ranges(self) -> None:
        for stem in BATCH3_STEMS:
            schema, example = load_contract(stem)
            payload = example.get("payload")
            if not isinstance(payload, dict):
    ...
    def test_experience_batch3_schemas_reject_non_string_array_items(self) -> None:
        exercised = 0
        for stem in BATCH3_STEMS:
            schema, example = load_contract(stem)
        ...
        self.assertGreater(exercised, 0, "no batch3 array fields were exercised")

    def test_experience_batch3_schemas_reject_payload_enum_escape_values(self) -> None:
        for stem in BATCH3_STEMS:
            schema, example = load_contract(stem)
            payload = example.get("payload")
            if not isinstance(payload, dict):
