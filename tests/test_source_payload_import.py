...
class SourcePayloadImportTests(unittest.TestCase):
    ...
    def _review_historical_postures(self, redistribution_posture: str = "metadata_only") -> None:
        review_path = self.repo / self.review_ref
        review = importer.load_json(review_path)
        review["source_posture_resolution"] = {
            "basis": "primary-license-private-retention",
            "server_processing_postures": ["not_authorized"],
            "redistribution_postures": [redistribution_posture],
            "rationale": "Primary license permits private retention; these are prior admission defaults.",
        }
        self._write_json(review_path, review)
        self.plan["rights_policy"]["rights_review"]["sha256"] = importer.sha256_file(review_path)
        self._write_json(self.plan_path, self.plan)
        self.context = importer.load_plan(self.repo, self.plan_path)

    def test_historical_postures_require_exact_explicit_source_review(self) -> None:
        rights = importer.load_json(self.repo / self.rights_ref)
        rights.update(server_processing_posture="not_authorized", redistribution_posture="metadata_only")
        self._refresh_rights_bindings(rights)
        with self.assertRaises(importer.RightsGateError):
            self._import_one()
        self.assertFalse(self.transport.put_calls)
        self._review_historical_postures()
        importer.validate_source_review(self.context)
        self.assertTrue(self._import_one().upload_attempted)

    def test_historical_resolution_cannot_waive_denial_restrictions_or_uncertainty(self) -> None:
        for field, value in (
            ("server_processing_posture", "denied"),
            ("redistribution_posture", "prohibited"),
            ("assessment_status", "research_only"),
            ("restrictions", ["No cloud storage permitted"]),
        ):
            with self.subTest(field=field):
                rights = importer.load_json(self.repo / self.rights_ref)
                rights.update(assessment_status="licensed", restrictions=[], server_processing_posture="not_authorized", redistribution_posture="metadata_only")
                rights[field] = value
                self._refresh_rights_bindings(rights)
                self._review_historical_postures()
                with self.assertRaises(importer.RightsGateError):
    ...
    def test_private_retention_requires_review_of_unadmitted_redistribution(self) -> None:
        rights = importer.load_json(self.repo / self.rights_ref)
        rights.update(server_processing_posture="not_authorized", redistribution_posture="not_authorized")
        self._refresh_rights_bindings(rights)
        with self.assertRaises(importer.RightsGateError):
            self._import_one()
        self.assertFalse(self.transport.put_calls)
        self._review_historical_postures(redistribution_posture="not_authorized")
        self.assertTrue(self._import_one().upload_attempted)

    def test_private_redistribution_resolution_cannot_authorize_public_delivery(self) -> None:
        rights = importer.load_json(self.repo / self.rights_ref)
        rights.update(server_processing_posture="not_authorized", redistribution_posture="not_authorized")
        self._refresh_rights_bindings(rights)
        self._review_historical_postures(redistribution_posture="not_authorized")
        self.plan["access_class"] = "public-payload"
        self._write_json(self.plan_path, self.plan)
        with self.assertRaises((importer.PlanError, importer.RightsGateError)):
            self.context = importer.load_plan(self.repo, self.plan_path)
            self._import_one()
        self.assertFalse(self.transport.put_calls)

    def test_private_redistribution_resolution_preserves_actual_restrictions(self) -> None:
        for posture in ("denied", "restricted", "prohibited", "unknown"):
            with self.subTest(posture=posture):
                rights = importer.load_json(self.repo / self.rights_ref)
                rights.update(server_processing_posture="not_authorized", redistribution_posture=posture)
                self._refresh_rights_bindings(rights)
                self._review_historical_postures(redistribution_posture="not_authorized")
                with self.assertRaises(importer.RightsGateError):
                    self._import_one()
    ...
    def test_historical_resolution_cannot_authorize_derivatives(self) -> None:
        rights = importer.load_json(self.repo / self.rights_ref)
        rights.update(server_processing_posture="not_authorized", redistribution_posture="metadata_only")
        self._refresh_rights_bindings(rights)
        self._review_historical_postures()
        self.plan["allowed_derivatives"]["embeddings"]["state"] = "allowed"
        self._write_json(self.plan_path, self.plan)
        # Schema may reject an unknown state before the transfer gate does.
        with self.assertRaises((importer.PlanError, importer.RightsGateError)):
            self.context = importer.load_plan(self.repo, self.plan_path)
            self._import_one()
        self.assertFalse(self.transport.put_calls)

    def test_conditional_license_requires_complete_retained_conditions(self) -> None:
        rights = importer.load_json(self.repo / self.rights_ref)
        rights.update(server_processing_posture="authorized_with_conditions",
                      redistribution_posture="authorized_with_conditions",
                      restrictions=["Retain author attribution.", "Retain license notice."])
        self._refresh_rights_bindings(rights)
        with self.assertRaises(importer.RightsGateError):
            self._import_one()
        review_path = self.repo / self.review_ref
        ...
        resolution = {
            "basis": "primary-license-private-retention",
            "server_processing_postures": ["authorized_with_conditions"],
            "redistribution_postures": ["authorized_with_conditions"],
            "rationale": "Unmodified private byte retention preserves the original notices.",
            "retained_conditions": [
                {"source_condition": "Retain author attribution.", "how_satisfied": "Original header remains byte-identical."}
            ],
        }
        review["source_posture_resolution"] = resolution
        for complete in (False, True):
            if complete:
                resolution["retained_conditions"].append({
                    "source_condition": "Retain license notice.",
                    "how_satisfied": "Original notice is preserved; source rights record retains its license link.",
                })
            self._write_json(review_path, review)
            self.plan["rights_policy"]["rights_review"]["sha256"] = importer.sha256_file(review_path)
            self._write_json(self.plan_path, self.plan)
