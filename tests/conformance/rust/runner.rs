...
use tos_foundation::{
    CanonicalProfile, CodePointSpan, Digest256, JsonLimits, JsonMode,
    JsonNumberKind, JsonValue, RelativePath, SourceRevision,
    canonical_bytes_v1, emit_preserved_json, parse_json,
};
use tos_source_store::{CorpusReader, ReadLimits, Selector, StoreErrorCode};

fn fixtures() -> PathBuf {
    let source = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(file!());
    source.parent().expect("conformance runner has a parent").to_path_buf()
}

fn lines(name: &str) -> Vec<Value> {
    fs::read_to_string(fixtures().join(name)).unwrap().lines()
        .map(|line| serde_json::from_str(line).unwrap()).collect()
}

fn required<'a>(value: &'a Value, key: &str) -> &'a str {
    value.get(key).and_then(Value::as_str).unwrap_or_else(|| panic!("missing {key}: {value}"))
}
        ...
        match required(&case, "operation") {
            "relative_path" => {
                ...
                if let Some(code) = expected.get("reject") {
                    assert_eq!(actual.unwrap_err().code.as_str(), code.as_str().unwrap(), "{id}");
                } else {
                    assert_eq!(actual.unwrap().as_str(), required(expected, "accept"), "{id}");
                }
            }
            "span" => {
                let text = required(&case, "input_utf8");
                assert_eq!(text.chars().count() as u64, expected["code_points"].as_u64().unwrap(), "{id}");
                assert_eq!(text.len() as u64, expected["utf8_bytes"].as_u64().unwrap(), "{id}");
                let range = expected["code_point_range"].as_array().unwrap();
                let span = CodePointSpan::new(
                    range[0].as_u64().unwrap(), range[1].as_u64().unwrap(),
                    Digest256::of_bytes(text.as_bytes()), "source-exact-v1",
                ).unwrap();
                let bytes = span.byte_span_in(text).unwrap();
                let result = expected["corresponding_byte_range"].as_array().unwrap();
                assert_eq!([bytes.start, bytes.end], [result[0].as_u64().unwrap(), result[1].as_u64().unwrap()], "{id}");
                assert!(span.byte_span_in("AéO").is_err(), "{id}: representation digest must bind offsets");
            }
            operation @ ("parse" | "parse_preserve" | "canonical") => {
                ...
                if operation != "canonical" && expected.get("reject").is_some() {
                    assert_eq!(parsed.unwrap_err().code.as_str(), required(expected, "reject"), "{id}");
                    continue;
                }
                let parsed = parsed.unwrap_or_else(|error| panic!("{id}: unexpected parse error {error}"));
                if operation == "canonical" {
                    assert_eq!(required(&case, "profile"), "CorpusSnapshotV1", "{id}");
                    let canonical = canonical_bytes_v1(
                        parsed.root(), CanonicalProfile::CorpusSnapshotV1, JsonLimits::default(),
                    );
                    if expected.get("reject").is_some() {
                        assert_eq!(canonical.unwrap_err().code.as_str(), required(expected, "reject"), "{id}");
                    } else {
                        let bytes = canonical.unwrap();
                        assert_eq!(bytes.as_slice(), required(expected, "canonical_utf8").as_bytes(), "{id}");
                        assert_eq!(Digest256::of_bytes(&bytes).to_hex(), required(expected, "digest_hex"), "{id}");
                    }
                    continue;
                }
                if let Some(order) = expected.get("member_order") {
                    let observed: Vec<&str> = parsed.root().as_object().unwrap().iter()
                        .map(|(key, _)| key.as_str().unwrap()).collect();
                    let wanted: Vec<&str> = order.as_array().unwrap().iter().map(|key| key.as_str().unwrap()).collect();
                    assert_eq!(observed, wanted, "{id}");
                }
                if let Some(numbers) = expected.get("numbers").and_then(Value::as_object) {
                    for (key, wanted) in numbers {
                        let JsonValue::Number(number) = parsed.root().object_get(key).unwrap() else {
                            panic!("{id}: {key} lost numeric context");
                        };
                        let kind = match number.kind { JsonNumberKind::Int => "Int", JsonNumberKind::Float => "Float" };
                ...
                if let Some(wanted) = expected.get("preserved_utf8") {
                    ...
                    assert_eq!(actual.as_slice(), wanted.as_str().unwrap().as_bytes(), "{id}");
                }
            }
...
fn fixture_store() -> PathBuf { fixtures().join("corpus-v1/store") }
...
fn copy_tree(from: &Path, to: &Path) -> io::Result<()> {
    ...
    for entry in fs::read_dir(from)? {
        ...
        if entry.file_type()?.is_dir() { copy_tree(&source, &destination)?; }
        else { fs::copy(source, destination)?; }
    }
    Ok(())
}
...
fn selected_object(root: &Path, sha256: &str) -> PathBuf { root.join("objects").join(sha256) }
...
fn independent_corpus_v1_positive_and_missing() {
    ...
    assert_eq!(reader.select_current().unwrap().unwrap().0.to_hex(), required(&fixture, "revision_current"));
    for case in fixture["selected_cases"].as_array().unwrap() {
        let id = required(case, "case_id");
        let snapshot = reader.load_exact(revision(required(case, "revision"))).unwrap();
        ...
        if case.get("expected_error").is_some() {
            assert_eq!(format!("{:?}", result.unwrap_err().code), required(case, "expected_error"), "{id}");
            continue;
        }
        let actual = result.unwrap();
        let expected = &case["expected_descriptor"];
        assert_eq!(actual.revision.0.to_hex(), required(expected, "revision"), "{id}");
        assert_eq!(actual.path.as_str(), required(expected, "path"), "{id}");
        assert_eq!(actual.sha256.to_hex(), required(expected, "sha256"), "{id}");
        assert_eq!(actual.size_bytes, expected["size_bytes"].as_u64().unwrap(), "{id}");
        assert_eq!(actual.mode as u64, expected["mode"].as_u64().unwrap(), "{id}");
        let mut selected = Vec::new();
        let count = reader.read_selected(&snapshot, &actual, 1024, &mut selected).unwrap();
        assert_eq!(count as usize, selected.len(), "{id}");
        assert_eq!(selected, decode_hex(required(case, "expected_bytes_hex")), "{id}");
    }
}
...
fn independent_corpus_v1_negative_boundaries() {
    ...
    for case in fixture["negative_cases"].as_array().unwrap() {
        ...
        if matches!(id, "traversal" | "git-component") {
            assert_eq!(RelativePath::parse(required(case, "selector_path")).unwrap_err().code.as_str(),
                       "unsafe_path", "{id}");
            continue;
        }
        ...
        match id {
            ...
            "unrelated-corrupt-object" => {
                ...
                let manifest: Value = serde_json::from_slice(&fs::read(root.join("revisions").join(current).join("snapshot.json")).unwrap()).unwrap();
                let beta_sha = manifest["files"].as_array().unwrap().iter()
                    .find(|entry| entry["path"] == "records/beta.txt").unwrap()["sha256"].as_str().unwrap();
                fs::write(selected_object(&root, beta_sha), b"damaged unrelated object").unwrap();
            }
            ...
            "canonical-manifest-revision-mismatch" => {
                let mut manifest: Value = serde_json::from_slice(&fs::read(&old_snapshot).unwrap()).unwrap();
            ...
            "duplicate-member-path" => {
                ...
                assert_eq!(reader.load_exact(revision(&new_revision)).unwrap_err().code,
                           StoreErrorCode::InvalidMemberIndex, "{id}");
                continue;
            }
            "missing-old-revision" | "bounded-read" | "declared-selected-size-over-cap"
            | "actual-object-growth-after-resolve" | "manifest-byte-cap"
            | "aggregate-index-entry-cap" => {}
            other => panic!("unhandled corpus negative {other}"),
        }
        let mut limits = read_limits();
        if id == "declared-selected-size-over-cap" { limits.max_selected_object_bytes = 4; }
        if id == "manifest-byte-cap" { limits.max_manifest_bytes = 488; }
        if id == "aggregate-index-entry-cap" { limits.max_manifest_entries = 2; }
        let reader = CorpusReader::open_existing(&root, limits).unwrap();
        let result_code = if id == "missing-old-revision" {
            reader.load_exact(revision(required(case, "selected_revision"))).unwrap_err().code
        } else if id == "manifest-byte-cap" || id == "aggregate-index-entry-cap"
            || id == "corrupt-ancestor-manifest"
            || id == "duplicate-identity-json-field" || id == "canonical-manifest-revision-mismatch"
        {
            ...
            if id == "actual-object-growth-after-resolve" {
                let descriptor = reader.resolve(&snapshot, Selector::SourceId("tos.work.synthetic.alpha")).unwrap();
                fs::write(&old_object, b"larger").unwrap();
                let mut stage = Vec::new();
                let error = reader.read_selected(&snapshot, &descriptor, 5, &mut stage).unwrap_err();
                assert!(stage.is_empty(), "{id}: preflight refusal must not expose bytes");
                error.code
            } else if id == "bounded-read" {
                let descriptor = reader.resolve(&snapshot, Selector::SourceId("tos.work.synthetic.alpha")).unwrap();
                let mut stage = Vec::new();
                let error = reader.read_selected(&snapshot, &descriptor, 4, &mut stage).unwrap_err();
                assert!(stage.is_empty(), "{id}: declared size refusal must not expose bytes");
                error.code
            } else {
                let result = reader.resolve(&snapshot, Selector::SourceId("tos.work.synthetic.alpha"));
        ...
        assert_eq!(format!("{result_code:?}"), required(case, "expected_error"), "{id}");
    }
}
