"""Qualified membership attachment on tiny synthetic metadata, never the corpus."""
...
import source_collection_commands as attachment
...
class NativeCollectionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.collection_ref = 'ToS/source-witnesses/collections/synthetic/parent/collection.json'
        self.work_ref = 'ToS/source-witnesses/works/synthetic/membership/work.json'
        self.collection = {'schema_version': 'tos_corpus_record_v1', 'record_type': 'collection',
            'record_id': 'tos.collection.synthetic.english', 'record_version': 1,
            'preferred_label': 'Synthetic English collection', 'variant_labels': [], 'identity_status': 'provisional',
            'source_refs': ['https://example.invalid/reported-attribution'], 'external_identifiers': [],
            'same_as_posture': 'no_equivalence_claim', 'membership_claim_refs': [], 'supersedes_ref': None,
        ...
        self.work = {key: copy.deepcopy(self.collection[key]) for key in ('schema_version', 'record_version',
        ...
        self.work.update(record_type='work', record_id='tos.work.synthetic.membership', preferred_label='Synthetic member Work', expression_claim_refs=[])
        self.write(self.collection_ref, self.collection)
        self.write(self.work_ref, self.work)
        self.collection_path = self.root / self.collection_ref
        ...
        self.owner = self.root / 'membership-owner.json'
        self.config = {'schema_version': attachment.CONFIG, 'uid': os.getuid(), 'principal_id': 'model:synthetic',
            'maker_type': 'model', 'source_root': str(self.root), 'authority_ref': 'test-only:membership-attachment',
            'expires_at': '2099-01-01T00:00:00Z', 'allowed_operations': [attachment.OPERATION, attachment.RECOVERY],
            'collection_id': self.collection['record_id'], 'collection_source_path': self.collection_ref,
            'work_id': self.work['record_id'], 'work_source_path': self.work_ref, 'predicate': 'contains_work',
            'allowed_collection_form_ids': ['tos.form.synthetic.collection.name'],
            'allowed_evidence_refs': ['https://example.invalid/reported-attribution'], 'retained_membership_provenance_refs': []}
        self.select_claim('first')
        self.forms = [{'form_id': self.config['allowed_collection_form_ids'][0], 'field_id': 'metadata.preferred-name'}]
        value, _, _ = attachment._forms(self.collection, None, self.forms, self.config['principal_id'])
        self.write(self.collection_path.with_name('collection.human-forms.json').relative_to(self.root).as_posix(), value)
        self.original_forms = copy.deepcopy(value)
        self.untouched = self.collection_path.parent / 'unrelated-note.txt'
        self.untouched.write_bytes(b'Unrelated metadata must not be touched.')
        self.rebuild()
    ...
    def select_claim(self, suffix):
        self.config.update(claim_id='tos.claim.synthetic.membership.' + suffix,
            claim_source_path='ToS/source-witnesses/relations/synthetic-membership-' + suffix + '/source-claims.jsonl',
            provenance_event_id='tos.event.synthetic.membership.' + suffix,
            allowed_claim_form_ids=['tos.form.synthetic.membership.' + suffix + '.statement'])
    ...
    def claim(self):
        return {'schema_version': 'tos_source_relation_claim_v1', 'claim_id': self.config['claim_id'],
            ...
            'predicate': 'contains_work', 'subject_ref': self.config['collection_id'], 'object': self.config['work_id'],
            ...
            'assessment_refs': [], 'qualifiers': {'statement': 'The selected provider attributes this translation to the selected person.',
                ...
                'membership_scope': 'Provider attribution only; later electronic editing is not attributed to the membership.',
    ...
    def proposal(self):
        return {'schema_version': attachment.REQUEST, 'operation': attachment.PREPARE,
            'work': copy.deepcopy(self.work), 'claim': self.claim(), 'forms': self.forms,
            'claim_forms': [{'form_id': self.config['allowed_claim_form_ids'][0], 'field_id': 'claim.statement'}],
            'reason': 'Synthetic qualified membership attribution without source admission.'}
    ...
    def rebuild(self):
        records, claims = {'collection': [], 'work': []}, []
        for path in (self.collection_path, self.work_path):
    ...
    def test_prepare_attach_current_read_and_fresh_process_replay(self):
        ...
        self.assertEqual(json.loads(self.collection_path.read_bytes()), {**self.collection, 'record_version': 2,
            'membership_claim_refs': [self.config['claim_id']]})
        self.assertEqual(self.work_path.read_bytes(), work_before)
        self.assertEqual(self.untouched.read_bytes(), untouched)
        forms = json.loads(self.collection_path.with_name('collection.human-forms.json').read_bytes())
        ...
        self.assertEqual(verified['receipt']['work_source_binding']['source_sha256'], commands._digest(work_before))
    ...
    def test_competing_claims_keep_distinct_identity_and_exact_append_order(self):
        ...
        with self.assertRaises(ValueError):
            validate_collection_membership_closure(collection, {self.work['record_id']: self.work}, [first['claim'], first['claim']])

    def test_current_grant_packet_and_qualified_delta_are_fail_closed(self):
        before = self.collection_path.read_bytes()
        alterations = [
            lambda p: p['work'].update(preferred_label='Injected alias'),
            lambda p: p['work'].update(record_id='tos.work.synthetic.other'),
            lambda p: p['claim'].update(object='tos.work.synthetic.other'),
            ...
            lambda p: p['claim']['qualifiers'].update(membership_scope=''),
    ...
    def test_pending_authority_dependency_and_third_state_are_revalidated(self):
        pending = self.crash(self.request(), edge=4)
    ...
    def test_native_membership_requires_exact_committed_capture(self):
        ...
        issues = []
        self.assertEqual(_native_membership_claims(self.root, issues), [])
    ...
    def correct_work(self):
        config = {key: self.config[key] for key in ('uid', 'principal_id', 'source_root', 'authority_ref', 'expires_at')}
        config.update(schema_version=commands.CORPUS_SELECTED_REVISION_CONFIG, record_type='work',
            record_id=self.config['work_id'], source_path=self.work_ref, allowed_fields=['notes'],
            allowed_operations=['record.revise'], allowed_form_ids=['tos.form.synthetic.work.name'])
        owner = self.root / 'work-correction-owner.json'
    ...
    def test_work_successor_resolves_exact_initial_bytes_and_requires_committed_history(self):
        ...
        self.assertEqual(old_collection['record'], self.collection)
        old_work = reader.resolve_source_bytes(self.work_ref, hashlib.sha256(work_raw).hexdigest())
        self.assertEqual(old_work['status'], 'available', old_work)
        self.assertEqual(old_work['record'], self.work)
        reader.verify_current()
        work_history = json.loads(self.work_path.with_name(revisions.HISTORY).read_bytes())
        identifier = work_history['receipts'][0]['publication']['transaction_id']
        ...
        history_path = self.work_path.with_name(revisions.HISTORY)
