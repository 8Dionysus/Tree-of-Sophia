...
TREE_BOUNDED_HOP_REL = Path("ToS/canon/concept/becoming/node.json")
...
TREE_WORKED_ANALOGY_REL = Path(
    "ToS/canon/analogy/friedrich-nietzsche/thus-spoke-zarathustra/prologue-1/bee-honey-analogy/node.json"
)
TREE_WORKED_SYNTHESIS_REL = Path(
    "ToS/canon/synthesis/friedrich-nietzsche/thus-spoke-zarathustra/prologue-1/departure-from-reflective-origin/node.json"
)

EXAMPLE_SOURCE_NODE_REL = Path("ToS/public-compatibility/source_node.example.json")
EXAMPLE_BOUNDED_HOP_REL = Path("ToS/public-compatibility/concept_node.example.json")
EXAMPLE_WORKED_PRINCIPLE_REL = Path("ToS/public-compatibility/principle_node.example.json")
EXAMPLE_WORKED_LINEAGE_REL = Path("ToS/public-compatibility/lineage_node.example.json")
EXAMPLE_WORKED_EVENT_REL = Path("ToS/public-compatibility/event_node.example.json")
EXAMPLE_WORKED_STATE_REL = Path("ToS/public-compatibility/state_node.example.json")
EXAMPLE_WORKED_SUPPORT_REL = Path("ToS/public-compatibility/support_node.example.json")
EXAMPLE_WORKED_ANALOGY_REL = Path("ToS/public-compatibility/analogy_node.example.json")
EXAMPLE_WORKED_SYNTHESIS_REL = Path("ToS/public-compatibility/synthesis_node.example.json")
...
def mirror_pairs(repo_root: Path | None = None) -> list[tuple[Path, Path]]:
    ...
    return [
        ...
        (root / TREE_WORKED_ANALOGY_REL, root / EXAMPLE_WORKED_ANALOGY_REL),
        (root / TREE_WORKED_SYNTHESIS_REL, root / EXAMPLE_WORKED_SYNTHESIS_REL),
    ]
