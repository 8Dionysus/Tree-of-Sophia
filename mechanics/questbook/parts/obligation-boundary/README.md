# Obligation Boundary

## Operating Card

| Field | Route |
| --- | --- |
| role | keep ToS obligations operational without replacing philosophy |
| input | obligation pressure, source-first boundary, quest route need |
| output | obligation boundary doctrine |
| owner | `mechanics/questbook/parts/obligation-boundary/` |
| next route | root `QUESTBOOK.md` or root `quests/` when public records change |
| tools | boundary doc and questbook route card |
| check | `python scripts/validate_mechanics_topology.py` |

## Payload

- `docs/QUESTBOOK_TOS_INTEGRATION.md`
