...
uses the full in-memory view material for selection diagnostics. Snapshot and
review-packet fingerprints and per-layer review counts are computed from the
exact global records reached through the exported `node_ids` and `edge_ids`, so
a consumer can reproduce them from the v2 payload. Validators require every
view reference to resolve inside the global materialized set and independently
recompute every view fingerprint and layer summary. `abyss-stack` remains the
runtime owner and must dereference view membership without writing runtime
state back into ToS.
