...
Rows marked `lab-only` are deterministic synthetic A/B/C controls for the
named contract. For a real witness, frozen text, or translation change, also
run the unflagged foundation validator and the exact route-local check; a
green lab probe does not inspect or accept the real record.

| Question | Read in this order | Narrow check |
| --- | --- | --- |
| Can I identify a source and preserve rights/provenance? | [`CORPUS_FOUNDATION.md`](doctrine/CORPUS_FOUNDATION.md) -> [`source-witnesses/README.md`](source-witnesses/README.md) -> [`source-witnesses/AGENTS.md`](source-witnesses/AGENTS.md) -> exact witness | `python scripts/validate_source_witness_foundation.py` |
| Can I freeze text units or segmentation? | [`SOURCE_TEXT_UNIT_SEGMENTATION_RESEARCH_2026-08-11.md`](research-packets/foundation-laboratory-2026-07/SOURCE_TEXT_UNIT_SEGMENTATION_RESEARCH_2026-08-11.md) -> [`source-text-unit-packet-v1.schema.json`](contracts/source-text-unit-packet-v1.schema.json) -> exact frozen text layer, review, or export | `python scripts/validate_source_witness_foundation.py` and `python scripts/validate_source_witness_foundation.py --source-text-unit-v1-lab-only` |
| Can I compare a translation or competing alignment? | [`TRANSLATION_ALIGNMENT_IDENTITY_RESEARCH_2026-08-11.md`](research-packets/foundation-laboratory-2026-07/TRANSLATION_ALIGNMENT_IDENTITY_RESEARCH_2026-08-11.md) -> [`translation-alignment-packet-v1.schema.json`](contracts/translation-alignment-packet-v1.schema.json) -> exact source/target witness, review, or export | `python scripts/validate_source_witness_foundation.py` and `python scripts/validate_source_witness_foundation.py --translation-alignment-v1-lab-only` |
| Is an interpretation still provisional in the bounded current pack? | [`doctrine/KNOWLEDGE_MODEL.md`](doctrine/KNOWLEDGE_MODEL.md) -> [`candidate-intake/AGENTS.md`](candidate-intake/AGENTS.md) -> [`prologue-1/mode-b/README.md`](candidate-intake/thus-spoke-zarathustra/prologue-1/mode-b/README.md) -> source anchor | `python scripts/validate_intake_pack.py` (only the current `thus-spoke-zarathustra/prologue-1/mode-b` pack); for another pack use its pass-local validator |
| What was accepted, rejected, deferred, or left contested? | [`review-ledger/AGENTS.md`](review-ledger/AGENTS.md) -> exact review -> [`canon/AGENTS.md`](canon/AGENTS.md) | `python scripts/validate_tos_source_home.py` and `python scripts/validate_nested_agents.py`; if canon also changes, run `python scripts/validate_tree_node_contracts.py` and `python mechanics/relation-weaving/parts/graph-promotion/scripts/validate_tree_relation_pack.py` |
...
| What may a person or downstream system consume through the bounded tiny-entry/KAG seam? | [`zarathustra/public-entry/TINY_ENTRY_ROUTE.md`](zarathustra/public-entry/TINY_ENTRY_ROUTE.md) -> public compatibility -> derived export | `python scripts/validate_tiny_entry_route.py`, `python scripts/validate_root_entry_map.py`, and `python mechanics/boundary-bridge/parts/derived-kag-seam/scripts/validate_kag_export.py`; for another touched export, run its builder `--check` and validator named by [`derived-exports/AGENTS.md`](derived-exports/AGENTS.md) |
