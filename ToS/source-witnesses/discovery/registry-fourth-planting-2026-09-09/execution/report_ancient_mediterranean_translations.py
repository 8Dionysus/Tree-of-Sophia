...
def report(completed):
    ...
    if not completed:
        ...
        write(BASE / 'local-presence.preparation.json', {'observed_at': now, 'scope': 'this checkout before fourth acquisition', 'targets': observations})
    ...
    for index, target in enumerate(manifest['targets'], 1):
        ...
        entry = {'target_id': f'registry-fourth-planting.{index:02}.{target["slug"]}', 'preferred_label': target['title'],
    ...
    result = {'schema_version': 'tos_registry_first_planting_result_v1', 'completed_at': now,
        ...
        'scope': '59 English translations of existing Works in A25/A26/A29; Greek versions preserved and parallel-linked.',
    ...
    lines = ['# Четвёртая посадка: английские переводы', '',
        ...
        '35 произведений платоновского корпуса, 14 Плутарха, 7 аристотелевского корпуса, 2 Эпиктета и Диоген Лаэртский. У каждого сохранены собственные переводчик, издание и проверенный локальный файл; рядом доступна прежняя греческая версия.', '',
    ...
    lines.extend(['', '## Границы', '',
        'Девять альтернативных английских переводов сохранены как метаданные для отдельного выбора версии. Ещё у четырёх прежних греческих Works нет английского файла в проверенном снимке этого репозитория; это не утверждение об отсутствии переводов вообще.', '',
        'У «Экономики» исходный CTS ошибочно помечает английскую версию как grc. Декларация языка и прочитанное начало самого TEI подтверждают английский. Противоречие и проверенный фрагмент сохранены в [разборе](SOURCE_AND_RIGHTS_REVIEW.md#economics-conflicting-upstream-language-metadata); исходные bytes не исправлялись.', '',
    ...
    for branch in sorted({plan['anchor_preparation']['branch_path'] for plan in branches.values()}):
        ...
        for path in [branch_root / 'README.md', branch_root / 'sources/README.md']:
            ...
            marker = '## Registry planting: ancient Mediterranean English-translation (2026-09-09)'
            if marker in prior:
                continue
