"""Fourth planting: review translation metadata before acquiring complete files."""
...
def prepare():
    ...
    # One explicitly represented translation per existing Work. The lower
    # version suffix selects Loeb where paired with Goodwin, and Long for Epictetus.
    ...
    for work_urn, row in selected:
        ...
        target = {'slug': slug, 'work_slug': work_slug, 'title': title, 'work_title': work_record['preferred_label'],
            ...
            'coverage': {'kind': 'perseus-tei-translation', 'citation_scope': 'hierarchical_divisions',
                'cts_urn': urn, 'header_prefix_sha256': digest(header)},
    ...
    if len(targets) != 59 or len({target['ids']['work'] for target in targets}) != 59:
