...
def inspect_payloads(target: dict, bodies: list[tuple[dict, bytes]]) -> dict:
    ...
    if coverage["kind"] in {"bilara-root", "bilara-translation"} and any(
        key in coverage for key in ("reviewed_body_prefix_sha256", "reviewed_body_prefix_bytes")
    ):
        length = coverage.get("reviewed_body_prefix_bytes")
        if (len(bodies) != 1 or type(length) is not int or not 0 < length <= len(bodies[0][1])
                or sha256(bodies[0][1][:length]) != coverage.get("reviewed_body_prefix_sha256")):
            raise ValueError("Bilara source opening differs from the reviewed evidence")
    if coverage["kind"] in {"perseus-tei-work", "perseus-tei-translation", "perseus-tei-latin-work"}:
    ...
    elif coverage["kind"] == "bilara-translation":
        if target.get("language") != "en" or target.get("expression_role") != "translation":
            raise ValueError("Bilara English translation requires its explicit language and role")
        if type(coverage.get("file_count")) is not int or coverage["file_count"] != 1 or len(bodies) != 1:
            raise ValueError("Bilara translation requires exactly one prepared file")
        entry, body = bodies[0]
        uid = coverage.get("uid")
        if (not isinstance(uid, str) or not uid or ":" in uid
                or entry["basename"] != f"{uid}_translation-en-sujato.json"):
            raise ValueError("Bilara translation filename or supplied UID differs")
        ...
        if not isinstance(parsed, dict) or not parsed or any(not isinstance(value, str) for value in parsed.values()):
            raise ValueError("Bilara translation is not a nonempty segment-to-string mapping")
        if (not any(value.strip() for value in parsed.values())
                or any(not key.startswith(uid + ":") or not key[len(uid) + 1:] for key in parsed)):
            raise ValueError("Bilara translation segment identity or nonempty-text check failed")
        report["files"].append({"basename": entry["basename"], "uid": uid, "segment_count": len(parsed),
            "nonempty_segment_count": sum(bool(value.strip()) for value in parsed.values())})
        report["unique_segment_count"] = len(parsed)
        report["coverage_limit"] = "Exact supplied translation file and segment identifiers; no segment alignment or translation-quality assessment."
