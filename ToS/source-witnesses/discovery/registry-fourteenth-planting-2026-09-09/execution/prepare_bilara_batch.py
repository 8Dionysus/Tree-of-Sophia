...
def prepare(translated):
    ...
    if translated:
        ...
        if len(existing)!=271:raise ValueError('Pali Work closure incomplete')
    ...
    branch_ref='ToS/philosophy/source-planting-preparation/early-buddhist-sn1-11-'+('translations' if translated else 'pali')+'-20260909.json'
    ...
    write(base/'selection-review.json',{'reviewer_ref':'model:codex','observed_at':observed,'candidates':selections,'selected':expected,'deferred':0,'reused_existing':0,'new_source_bodies_acquired':False})
