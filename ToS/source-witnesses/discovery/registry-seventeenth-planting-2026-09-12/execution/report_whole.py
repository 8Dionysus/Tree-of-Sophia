...
expected={Path(row['path']).name.split('_')[0] for row in read(BASE/'candidate-files.json') if row['language']=='pli'}
assert len(expected)==1997
...
assert len(rows)==1997 and new_bytes==9660347
whole={'completed_at':datetime.now(timezone.utc).isoformat(),'scope':'Two stages of the seventeenth planting: 1,997 new Pali versions and 1,997 English Sujato translations, sharing 1,997 newly created Work identities','stage_results':['batch-result.json','translations/batch-result.json'],'new_works':1997,'new_versions':3994,'new_files':3994,'new_bytes':new_bytes,'paired_works':1997,'reused_Pali_versions':0,'pairs':rows,'deferred_files_ref':'deferred-files.json','source_text_admitted':False,'translation_quality_assessed':False,'semantic_alignment_admitted':False,'public_payload_delivery':False,'AI_derivative_use_admitted':False}
(BASE/'whole-batch-result.json').write_text(json.dumps(whole,ensure_ascii=False,indent=2)+'\n')
lines=['# Семнадцатая поставка: раннебуддийские AN 1–11 и SN 36–56','','**3 994 выбранные версии, 3 994 файла, 9 660 347 байт.** Добавлено 1 997 новых произведений. Каждое произведение представлено на пали и в английском переводе Суджато.','','Это 1 997 отдельно обозначенных сутт из AN 1–11 и SN 36–56 выбранной цифровой редакции. 174 групповых UID (348 файлов двух языков) отложены до отдельного описания состава; полнота AN 1–11 и SN 36–56 не заявлена. Оно не означает полноту древней устной традиции, всех редакций или Āgama-параллелей.','','| UID | Палийское название | Английское название | Обе версии |','| --- | --- | --- | --- |']
...
(BASE/'RESULTS.md').write_text('\n'.join(lines));(BASE/'README.md').write_text('# Seventeenth registry planting\n\n[1,997 paired reading routes and whole result](RESULTS.md). [Source and rights review](SOURCE_AND_RIGHTS_REVIEW.md).\n\n[1,997 new Pali versions](PALI_RESULTS.md) precede [1,997 English Work extensions](translations/RESULTS.md). Each stage retains its original preparation, checkpoint, custody and form history.\n')
print(json.dumps({'new_versions':3994,'new_works':1997,'paired_works':1997,'bytes':new_bytes}))
