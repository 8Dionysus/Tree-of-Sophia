...
base=root/'ToS/source-witnesses/discovery/registry-sixth-planting-2026-09-09'
manifest=json.loads((base/'manifest.json').read_text())
plans={x['target_slug']:x for x in json.loads((root/'ToS/philosophy/source-planting-preparation/roman-philosophical-english-translations-20260909.json').read_text())['targets']}
for target in manifest['targets']:
 ...
 if marker not in prior:
  latin.write_text(prior.rstrip()+'\n\n'+marker+'\n\n[English version and source file]('+os.path.relpath(out/'README.md',latin.parent)+'). Shared Work identity does not establish edition dependence or alignment.\n')
