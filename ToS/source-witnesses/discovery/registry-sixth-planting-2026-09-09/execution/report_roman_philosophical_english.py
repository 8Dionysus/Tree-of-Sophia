...
def report(completed):
    ...
    if not completed:
        ...
        for target in manifest['targets']:
            ...
            observations.append({'target_slug': target['slug'], 'new_payload_paths': paths, 'new_payloads_present': False,
                ...
                'parallel_Latin_payloads_present': all((ROOT / path).is_file() for path in target['parallel_source']['files'])})
        write(BASE / 'local-presence.preparation.json', {'observed_at': now, 'scope': 'this checkout before sixth acquisition', 'targets': observations})
    ...
    for index, target in enumerate(manifest['targets'], 1):
        ...
        readiness = {
            ...
            'branch': control([planting_ref, reading_ref] if completed else [manifest['branch_preparation_ref']],
                'Exact English version branch route and parallel Latin reading links are present.' if completed else 'Current branch and corpus anchor reviewed for an additional translation of the existing Work.'),
        }
        ...
        entry = {'target_id': f'registry-sixth-planting.{index:02}.{target["slug"]}', 'preferred_label': target['title'],
    ...
    for branch in sorted({plan['anchor_preparation']['branch_path'] for plan in branches.values()}):
        ...
        for path in [branch_root / 'README.md', branch_root / 'sources/README.md']:
            ...
            marker = '## Registry planting: Roman philosophical English-translation (2026-09-09)'
            if marker in prior:
                continue
            added = '\n\n' + marker + '\n\n' + f'{len(entries)} English versions alongside existing Latin texts. [Batch reading routes and evidence]({os.path.relpath(BASE / "RESULTS.md", path.parent)}).\n'
            if path.parent.name == 'sources':
                added += '\n' + '\n'.join('- [' + row['title'] + ' — English / Latin](' + os.path.relpath(ROOT / row['reading_route'], path.parent) + ')' for row in entries) + '\n'
