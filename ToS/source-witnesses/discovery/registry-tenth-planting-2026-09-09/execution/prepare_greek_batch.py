"""Tenth planting: bounded source-visible metadata before exact acquisition."""
...
def selection_reason(row):
    if row["path"].split("/")[2] in ("tlg051", "tlg052"):
        return "Defer combined Life carrier: source-witnesses owner must review the exact constituent/membership model for Agis and Cleomenes or Tiberius and Caius Gracchus before acquisition; no undifferentiated Work is introduced."
    return None
...
def openings():
    for row in candidate_files():
        ...
        url = f'https://raw.githubusercontent.com/{REPO}/{PIN}/{row["path"]}'
        ...
        write(path.with_name(path.name + '.receipt.json'), {'url': url, 'final_url': final, 'http_status': status,
            ...
            'retained_ref': path.relative_to(ROOT).as_posix(), 'retained_sha256': sha(body), 'retained_byte_size': len(body),
...
def prepare():
    ...
    families = {'tlg0059': ('plato', 'A25', 14, 1, 'Scaife Viewer / Perseus', 'A25-R001'),
        ...
        'tlg0007': ('plutarch', 'A29', 14, 10, 'Плутарх', 'A29-R130'),
    ...
    for row in candidate_files():
        ...
        work_slug = re.sub('[^a-z0-9]+', '-', title.lower()).strip('-')
        slug = family+'-'+work_slug+'-greek'
        ...
        item = 'git-tei-xml'
        w = f'ToS/source-witnesses/works/{family}/{work_slug}'
        ...
        if family == 'plutarch': limits += ['Reports of other schools are transmitting/polemical testimony, not accepted statements of those schools.', 'Lives, narratorial judgments and separately identified comparisons remain transmitted literary and historical testimony, not accepted biography or philosophical doctrine; distinct CTS comparisons are not merged into their paired Lives.', 'The supplied CTS title and language are retained literally; title normalization and person-name equivalence remain unreviewed.']
        if work in ('tlg013', 'tlg038', 'tlg039', 'tlg040'):
            limits += ['Unresolved supplied printing-date discrepancy: CTS edition description states 1914, TEI sourceDesc states 1916, both volume III/3. Exact CTS and Git identity is retained without adjudicating the print date.']
        if work in ('tlg021', 'tlg030', 'tlg041', 'tlg043', 'tlg055'):
            limits += ['Contributor presence differs across metadata carriers: sourceDesc omits a Perrin name. Retain exact titleStmt and CTS responsibility statements separately; do not fill missing bibliographic fields by series-wide inference.']
        if work == 'tlg053':
            limits += ['This separately identified comparison is not either combined Life carrier; no membership or pair relation to deferred tlg051/tlg052 is asserted by this intake.']
        ...
        target = {'slug': slug, 'work_slug': work_slug, 'title': title, 'family': family, 'provider': 'perseus', 'repository': REPO, 'pin': PIN,
    ...
    if len(targets) != 64 or len({t['slug'] for t in targets}) != len(targets):
