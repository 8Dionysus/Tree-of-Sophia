"""Third planting: bounded source-visible metadata before exact acquisition."""
...
def candidate_files():
    rows = []
    for name in ('perseus-tree.json', 'additional-candidate-tree.json'):
        rows += json.loads((BASE/'evidence'/name).read_text())['tree']
    plutarch = {f'tlg{i:03}' for i in (74, 89, 90, 92, 94, 102, 107, 126, 133, 134, 135, 136, 137, 138, 139, 140)}
    return sorted([r for r in rows if r['path'].endswith('.xml') and '.perseus-grc' in r['path']
        and (r['path'].split('/')[1] != 'tlg0007' or r['path'].split('/')[2] in plutarch)
        and not (r['path'].split('/')[1] == 'tlg0059' and int(r['path'].split('/')[2][3:]) <= 12)], key=lambda r: r['path'])
...
def selection_reason(row):
    ...
    if author == 'tlg0032':
        return 'Defer Xenophon: prepared metadata only; this pass uses exact existing corpus anchors, and A25 Perseus anchor specifically names Plato/Aristotle. Prepare the appropriate Socratic/political source anchor separately.'
    if author == 'tlg0059' and work == 'tlg036':
        return 'Defer Letters: prepare the epistolary collection and constituent identities instead of creating one undifferentiated Work.'
    if author == 'tlg0557' and work in ('tlg003', 'tlg004', 'tlg005'):
        return 'Defer fragment/gnomology packets: define excerpt, transmitting witness and scholarly compilation identities through the existing composite/fragment owner route.'
    if author == 'tlg0007' and work == 'tlg137':
        return 'Defer the short Compendium: registry A26-R061 confuses this CTS identifier with De communibus notitiis (tlg138); preserve the conflict for explicit registry correction.'
    return None
...
def prepare():
    ...
    for row in candidate_files():
        ...
        if family == 'plutarch': limits += ['The epitome remains distinct from its longer source work. Reports of other schools are transmitting/polemical testimony, not accepted statements of those schools.']
        ...
        target = {'slug': slug, 'title': title, 'family': family, 'provider': 'perseus', 'repository': REPO, 'pin': PIN,
            'language': 'grc', 'script': 'Grek', 'expression': exp, 'edition': edition, 'item': item,
    ...
    if len(targets) != 51 or len({t['slug'] for t in targets}) != len(targets):
        raise ValueError('selected count/identity differs from reviewed 51-edition scope')
    ...
    branch_ref = 'ToS/philosophy/source-planting-preparation/ancient-mediterranean-source-editions-20260908.json'
