...
base=root/'ToS/source-witnesses/discovery/registry-third-planting-2026-09-08'
manifest=json.loads((base/'manifest.json').read_text())
plans={x['target_slug']:x for x in json.loads((root/'ToS/philosophy/source-planting-preparation/ancient-mediterranean-source-editions-20260908.json').read_text())['targets']}
for target in manifest['targets']:
 ...
 record={'$schema':'https://tree-of-sophia.local/ToS/contracts/philosophy-source-planting.schema.json','schema_version':'tos_philosophy_source_planting_v1','planting_id':'tos.planting.'+anchor['atlas_row_id'].lower()+'.registry-'+target['slug'],'atlas_row_id':anchor['atlas_row_id'],'dossier_id':anchor['dossier_id'],'branch_path':anchor['branch_path'],'source_backlog_anchor':a,'source_witness':{'work_id':target['ids']['work'],'record_ref':target['paths']['work'],'relationship':'grounds_source_backlog_anchor'},'discovery_ref':'ToS/source-witnesses/discovery/runs/registry-'+target['slug']+'.2026-09-08.v1.json','provenance_event_ref':'tos.event.discovery.registry-20260908.'+target['slug'],'research_ref':str((base/'SOURCE_AND_RIGHTS_REVIEW.md').relative_to(root)),'fulfilled_source_needs':[plan['scope_rationale'],'Exact prepared version is locally retained: '+str(checked['files'])+' immutable files; Item manifest fixes SHA-256 and resource inventory records observed coverage.'],'remaining_controls':list(dict.fromkeys(plan['remaining_controls']+target['limits']+extra_limits+['Local file custody and branch route do not admit text, translation, semantics or canon.'])),'research_order':['official_and_classical_documentation','established_top_scholarship','fresh_current_relevance_check'],'authority':{'source_status':'metadata_witness_planted','review_status':'unreviewed','source_text_admitted':False,'semantic_status':'not_started','graph_status':'not_promoted','canon_status':'not_canon','human_task_created':False},'status':'source_witness_planted','maker':{'maker_type':'model','agent_ref':'model:codex','human_review_performed':False},'created_at':datetime.now(timezone.utc).isoformat(),'record_version':1}
