...
base=root/'ToS/source-witnesses/discovery/registry-seventh-planting-2026-09-09'
manifest=json.loads((base/'manifest.json').read_text())
plans={x['target_slug']:x for x in json.loads((root/'ToS/philosophy/source-planting-preparation/plutarch-epictetus-english-1874-20260909.json').read_text())['targets']}
for target in manifest['targets']:
 ...
 peer=target['parallel_translation']
 lines+=['## Earlier English version','', '[Earlier translation and owner records]('+link(peer['reading_route'])+').','',peer['relationship_limit'],'']
 (out/'README.md').write_text('\n'.join(lines))
 peer_path=root/peer['reading_route'];peer_text=peer_path.read_text();peer_marker='## Additional English version: '+target['slug']
 if peer_marker not in peer_text:
  peer_path.write_text(peer_text.rstrip()+'\n\n'+peer_marker+'\n\n[Additional English version]('+os.path.relpath(out/'README.md',peer_path.parent)+'). No alignment or quality ranking is asserted.\n')
