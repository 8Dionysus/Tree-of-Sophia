"""Seventh planting: review translation metadata before acquiring complete files."""
...
def openings():
    ...
    print('Nine incomplete source openings retained for review')
...
def prepare():
    ...
    prior_translations = {t['ids']['work']: t for t in json.loads((BASE.parent / 'registry-fourth-planting-2026-09-09/manifest.json').read_text())['targets']}
    ...
    # The nine exact previously deferred versions are reviewed independently here.
    ...
    for work_urn, row in selected:
        ...
        slug = work_slug + ('-english-1874' if family == 'plutarch' else '-english-1890')
        ...
        if not candidates_for_work:
            fallback = {'plato': 'A25-R001', 'aristotle': 'A25-R002', 'plutarch': 'A29-R113',
        ...
        previous = prior_translations[old['ids']['work']]
        target['parallel_translation'] = {'work_ref': work_ref, 'expression_ref': previous['paths']['expression'],
            'edition_ref': previous['paths']['edition'], 'item_manifest_ref': previous['paths']['item_root'] + '/item.manifest.json',
            'reading_route': old_anchor['branch_path'] + '/sources/plantings/registry-' + previous['slug'] + '/README.md',
            'files': [previous['paths']['item_root'] + '/payload/' + f['basename'] for f in previous['files']],
            'relationship_limit': 'Distinct supplied English versions of the same Work; no accuracy ranking, alignment or translation dependence.'}
        target['limits'].extend(['This is an additional English version; the previously acquired English Expression and Greek source remain preserved.',
    ...
    if len(targets) != 9 or len({target['ids']['work'] for target in targets}) != 9:
        raise ValueError('reviewed scope must be 9 distinct existing Works and 9 new English versions')
    ...
    branch_ref = 'ToS/philosophy/source-planting-preparation/plutarch-epictetus-english-1874-20260909.json'
    write(ROOT / branch_ref, {'schema_version': 'tos_source_planting_preparation_batch_v1',
        'status': 'prepared-not-planted', 'review_scope': '9 additional English translations attached to existing Works and A29 branches',
    ...
    write(BASE / 'selection-review.json', {'reviewer_ref': 'model:codex', 'observed_at': observed,
        ...
        'no-English-file-in-pinned-view': missing, 'selection_scope': 'nine exact alternative English versions deferred in fourth planting, not a provider-wide absence inventory', 'resolves_prior_selection_review_ref': 'ToS/source-witnesses/discovery/registry-fourth-planting-2026-09-09/selection-review.json', 'complete_source_files_acquired': False})
