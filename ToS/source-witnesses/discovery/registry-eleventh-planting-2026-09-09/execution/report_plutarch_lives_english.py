...
def report(completed):
    ...
    if not completed:
        ...
        write(BASE / 'local-presence.preparation.json', {'observed_at': now, 'scope': 'this checkout before eleventh acquisition', 'targets': observations})
    ...
    for index, target in enumerate(manifest['targets'], 1):
        ...
        entry = {'target_id': f'registry-eleventh-planting.{index:02}.{target["slug"]}', 'preferred_label': target['title'],
    ...
    result = {'schema_version': 'tos_registry_first_planting_result_v1', 'completed_at': now,
        ...
        'scope': '64 English versions of Plutarch Lives Greek Works, preserving shared Work identity and parallel Greek reading routes.',
    ...
    lines = ['# Одиннадцатая посадка: английские переводы жизнеописаний и сопоставлений Плутарха', '',
        f"Добавлено {len(rows)} английских версий существующих произведений: {result['total_files']} файлов, {result['total_bytes']:,} байт. Новых Work не создано.", '',
    ...
    for row in rows:
        lines.append(f"| {row['work_title']} | {row['title']} | [Открыть]({os.path.relpath(ROOT / row['reading_route'], BASE)}) |")
    lines.extend(['', '## Границы', '',
        'В отбор вошли английские версии 64 произведений десятой партии. Объединённые носители tlg051/tlg052 по-прежнему требуют отдельного разбора состава и не входят в эту партию.', '',
        'В четырёх английских версиях независимо обнаружен конфликт CTS 1914 / header 1916: Fabius Maximus, Nicias, Crassus и их сопоставление. Обе даты сохранены. Сведения переводчика и замечания источника остаются сообщениями поставщика.', '',
        'Для 192 новых записей созданы формы метаданных; у 64 существующих Work формы обновлены с сохранением истории. Точное копирование полей не даёт семантического принятия. [Проверка форм](human-form-companions.json).', '',
    ...
    for branch in sorted({plan['anchor_preparation']['branch_path'] for plan in branches.values()}):
        ...
        for path in [branch_root / 'README.md', branch_root / 'sources/README.md']:
            ...
            marker = '## Registry planting: Plutarch Lives English (2026-09-09)'
            if marker in prior: continue
            added = '\n\n' + marker + '\n\n' + f'{len(entries)} English versions alongside existing Greek texts. [Reading routes and evidence]({os.path.relpath(BASE / "RESULTS.md", path.parent)}).\n'
