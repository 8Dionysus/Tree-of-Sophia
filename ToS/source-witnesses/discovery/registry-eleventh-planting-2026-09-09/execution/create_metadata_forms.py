...
def main():
    ...
    for target in manifest['targets']:
        for kind in ('work', 'expression', 'edition', 'item'):
            ...
            config = {'schema_version':'tos_local_source_command_owner_v1', 'uid':os.getuid(),
                ...
                'allowed_operations':['form.revise'] if kind == 'work' else ['form.create'], 'expires_at':(datetime.now(timezone.utc)+timedelta(hours=24)).isoformat()}
            ...
            if created or stale:
                ...
                command_id = 'registry-eleventh-source-forms:'+key
    ...
    result={'scope':'Only the exact four corpus records per plutarch-lives-english target; source-copy wording via owner describe/prepare/apply, no semantic admission',
