...
def report(completed):
    ...
    if not completed:
        ...
        write(BASE / 'local-presence.preparation.json', {'observed_at': now, 'scope': 'this checkout before eighth acquisition', 'targets': observations})
    ...
    for index, target in enumerate(manifest['targets'], 1):
        ...
        entry = {'target_id': f'registry-eighth-planting.{index:02}.{target["slug"]}', 'preferred_label': target['title'],
    ...
    lines = ['# Восьмая посадка: греческий корпус Moralia', '',
        ...
        'Этика, воспитание, политика, религиозные вопросы и природа. Традиционная атрибуция Плутарху сохранена как сообщение поставщика; спорное авторство не разрешено.', '',
    ...
    for branch in sorted({plan['anchor_preparation']['branch_path'] for plan in branches.values()}):
        ...
        for path in [branch_root / 'README.md', branch_root / 'sources/README.md']:
            ...
            marker = '## Registry planting: Plutarch Moralia Greek (2026-09-09)'
