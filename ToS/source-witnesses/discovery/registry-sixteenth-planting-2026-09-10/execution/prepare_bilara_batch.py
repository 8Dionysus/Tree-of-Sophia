"""Freeze separately owned SN23–35 Pali witnesses, then guarded English Sujato Work extensions."""
...
def prepare(translated):
    ...
    if translated:
        ...
        if len(existing)!=329:raise ValueError('Pali Work closure incomplete')
    ...
    for row in rows:
        ...
        uid=row['path'].split('_')[0]
        if not re.fullmatch(r'sn[1-9][0-9]*\.[1-9][0-9]*',uid) or uid.split('.')[0]!=row['district']:raise ValueError('SN UID/district mismatch')
        ...
        refs=[shared+'/source-observations.json',shared+'/batch-scope.json',shared+'/deferred-files.json',rel+'/manifest.json',rel+'/SOURCE_AND_RIGHTS_REVIEW.md',shared+'/SOURCE_AND_RIGHTS_REVIEW.md',*meta_refs,f'https://github.com/{REPO}/tree/{PIN}'];refs=list(dict.fromkeys(refs))
    ...
    branch_ref='ToS/philosophy/source-planting-preparation/early-buddhist-sn23-35-'+('translations' if translated else 'pali')+'-20260910.json'
    ...
    observations=[read(p) for p in sorted((BATCH_ROOT/'evidence').glob('*.receipt.json'))]+[read(ROOT/(p+'.receipt.json')) for p in read(BATCH_ROOT/'inherited-metadata-refs.json')]
    write(base/'selection-review.json',{'reviewer_ref':'model:codex','observed_at':observed,'candidates':selections,'selected':expected,'deferred':6,'reused_existing':0,'new_source_bodies_acquired':False,'deferred_files_ref':shared+'/deferred-files.json'})
