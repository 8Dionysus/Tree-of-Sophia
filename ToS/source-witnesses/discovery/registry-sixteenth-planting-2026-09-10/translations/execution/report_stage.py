...
def report(done):
    ...
    selection=read(BASE/'selection-review.json');forms=read(BASE/'human-form-companions.json');result={'schema_version':'tos_registry_first_planting_result_v1','completed_at':now,'preparation_commit':read(BASE/'preparation-checkpoint-receipt.json')['commit'],'normalization_snapshot':m['source_registry_snapshot_ref'],'planted':rows,'deferred':[x for x in read((BASE.parent if translated else BASE)/'deferred-files.json') if x['language']==('en' if translated else 'pli')],'reused_existing':[s for s in selection['candidates'] if s['decision']=='reuse-existing'],'blocked':[],'total_files':sum(x['files'] for x in rows),'total_bytes':sum(x['bytes'] for x in rows),'scope':stage+' stage of the 329-discourse SN 23–35 paired corpus','human_form_companions_ref':REL+'/human-form-companions.json','new_works_created':0 if translated else len(rows),'source_text_admitted':False,'payload_visibility':'local_only','public_or_remote_deployment':False}
    write(BASE/'batch-result.json',result)
    lines=['# Шестнадцатая поставка: '+('английские переводы Суджато' if translated else 'палийские дискурсы'),' ',f"Добавлено {len(rows)} версий: {result['total_files']} файлов, {result['total_bytes']:,} байт.",'', '| UID | Название | Чтение |','| --- | --- | --- |']
    ...
    for branch in sorted({p['anchor_preparation']['branch_path'] for p in plans.values()}):
        for path in [ROOT/branch/'README.md',ROOT/branch/'sources/README.md']:
            prior=path.read_text();marker='## Registry planting: sixteenth '+stage+' stage (2026-09-10)'
            if marker not in prior:path.write_text(prior.rstrip()+'\n\n'+marker+'\n\n'+f'{len(rows)} exact versions. [Reading routes and evidence]({os.path.relpath(BASE/"RESULTS.md",path.parent)}).\n')
