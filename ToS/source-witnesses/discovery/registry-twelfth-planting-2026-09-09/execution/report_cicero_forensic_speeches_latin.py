...
def report(completed):
    ...
    if not completed:
        ...
        write(BASE / 'local-presence.preparation.json', {'observed_at': now, 'scope': 'this checkout before twelfth acquisition', 'targets': observations})
    ...
    for index, target in enumerate(manifest['targets'], 1):
        ...
        readiness = {
            'version': control([REL + '/manifest.json', REL + '/SOURCE_AND_RIGHTS_REVIEW.md'], 'Exact Latin edition, CTS Work identity, contributor statement and file list reviewed.'),
            ...
            'branch': control([planting_ref, reading_ref] if completed else [manifest['branch_preparation_ref']],
                'Exact Latin edition branch and local reading routes are present.' if completed else 'Current branch and corpus anchor reviewed for this separately identified Work.'),
        }
        ...
        entry = {'target_id': f'registry-twelfth-planting.{index:02}.{target["slug"]}', 'preferred_label': target['title'],
        ...
        if completed:
            ...
            if planting['source_witness']['work_id'] != target['ids']['work'] or planting['discovery_ref'] != discovery_ref:
                raise ValueError('branch route does not identify this exact Latin edition intake')
            entry['execution'] = control([planting_ref, target['paths']['work'], target['paths']['expression'], item + '/item.manifest.json'],
                'Exact Latin version acquired and branch-linked; no textual or semantic admission.', status='completed')
            rows.append({**checked, 'title': target['title'], 'language': 'la',
    ...
    result = {'schema_version': 'tos_registry_first_planting_result_v1', 'completed_at': now,
        ...
        'scope': '31 separately identified Latin Cicero speeches through the A29 Ciceronian corpus anchor.',
    ...
    lines = ['# Двенадцатая посадка: латинские речи Цицерона', '',
        f"Добавлено {len(rows)} произведений и точных латинских версий: {result['total_files']} файлов, {result['total_bytes']:,} байт.", '',
        '31 самостоятельная CTS-запись речи. Судебные обвинения, похвала и политические суждения остаются текстом источника, без принятия их исторической истинности.', '',
        '| Произведение | Ветвь | Латинская версия |', '| --- | --- | --- |']
    for row in rows:
        lines.append(f"| {row['title']} | {row['atlas_row']} | [Открыть]({os.path.relpath(ROOT / row['reading_route'], BASE)}) |")
    lines.extend(['', '## Границы', '',
        'Циклы речей, собрания писем и альтернативные версии отложены до отдельного разбора идентичности. Для Cum Populo Gratias Egit сохранено расхождение дат: CTS 1911, TEI 1909. В In L. Calpurnium Pisonem сохранены два ненумерованных контейнера и 118 разделов; отсутствие номеров явно отмечено, XML не исправлялся.', '',
        'Для всех 124 новых записей созданы 248 форм метаданных через штатную команду владельца: точное копирование исходных полей, без семантического принятия. [Проверка форм](human-form-companions.json).', '',
        f"Отложено в пределах этой партии: {len(result['deferred'])} кандидатов: циклы речей, письма, альтернативные редакции и латинский Timaeus. Требуется отдельный разбор состава и идентичности у владельца source-witnesses. Другие версии не входили в отбор.", '',
    ...
    (BASE / 'README.md').write_text('# Twelfth registry planting\n\n[Latin reading routes and result](RESULTS.md). [Source and rights review](SOURCE_AND_RIGHTS_REVIEW.md). [Selection](selection-review.json).\n\nPrepared readiness and post-acquisition completion retain separate evidence.\n')
    for branch in sorted({plan['anchor_preparation']['branch_path'] for plan in branches.values()}):
        ...
        for path in [branch_root / 'README.md', branch_root / 'sources/README.md']:
            ...
            marker = '## Registry planting: Cicero forensic-speeches Latin (2026-09-09)'
            if marker in prior: continue
            added = '\n\n' + marker + '\n\n' + f'{len(entries)} exact Latin versions. [Batch reading routes and evidence]({os.path.relpath(BASE / "RESULTS.md", path.parent)}).\n'
