"""Fifth planting: Latin source editions with exact reviewed identity carriers."""
...
def candidate_files():
    rows=json.loads((BASE/'evidence/perseus-tree.json').read_text())['tree']
    return sorted([r for r in rows if r['path'].endswith('.xml') and '.perseus-lat' in r['path']
        and ((r['path'].split('/')[1] == 'phi0474' and 36 <= int(r['path'].split('/')[2][3:]) <= 55)
        or r['path'].split('/')[1] == 'phi0550'
        or (r['path'].split('/')[1] == 'phi1017' and int(r['path'].split('/')[2][3:]) >= 11))],key=lambda r:r['path'])
...
def selection_reason(row):
    ...
    reasons={
        'phi0474.phi046.perseus-lat2.xml': 'Lucullus is separately catalogued but belongs to the complex Academica redaction history. Defer to source-witness identity review for an explicit Work/part/recension relation; do not create an unrelated duplicate from a CTS grouping alone.',
        'phi0474.phi053.perseus-lat2.xml': 'Retain the alternative TEI P4 metadata; select the reviewed P5 Mueller version lat1 in this batch. Another textual edition and legacy encoding require their own comparison and adapter review.',
        'phi1017.phi011.perseus-lat2.xml': 'Apocolocyntosis is a satirical work. Defer to A29 source owner for a specific satire/political reception anchor, rather than treating the prose corpus label as a genre judgment.',
        'phi1017.phi015.perseus-lat2.xml': 'The Epistulae Morales file contains an epistolary collection. Prepare collection and constituent letter identity under the existing source-owner route before intake, consistent with the earlier deferral of Plato Letters.'}
...
def prepare():
    ...
    families={'phi0474':('cicero',6,'Цицероновский корпус','A29-R027'),
        'phi0550':('lucretius',7,'De rerum natura','A29-R041'),
        'phi1017':('seneca',8,'Сенека','A29-R071')}
    ...
    for row in candidate_files():
        ...
        slug=re.sub('[^a-z0-9]+','-',title.lower()).strip('-');stem=Path(row['path']).stem
        ...
        w=f'ToS/source-witnesses/works/{family}/{slug}';e=f'{w}/expressions/{exp}';d=f'{e}/editions/{edition}';it=f'{d}/items/{item}';identity=family+'.'+slug
        ...
        if work in ('phi043','phi045') and family=='cicero':limits+=['The supplied surviving or reconstructed text does not restore lost ancient books. Academica/Lucullus redaction and part relations remain unasserted; the separately supplied Lucullus is deferred.']
        if family=='lucretius':limits+=['The Latin header explicitly says Lost information for its print/source bibliography and the CTS edition description is a dash. The underlying print exemplar/editor remain unknown; no modern editor, critical edition or public-domain term is inferred.']
        ...
        target={'slug':slug,'title':title,'family':family,'provider':'perseus','repository':REPO,'pin':PIN,'operation_date':'2026-09-09',
        ...
        for layer in rights['layer_assessments']:
            ...
            if family=='lucretius':layer['uncertainty']='Supplier license scope only. Print exemplar/editor absent in supplied metadata; not independently identified. No public-domain term or human legal review claimed.'
    ...
    if len(targets)!=21 or len({t['slug'] for t in targets})!=21:raise ValueError('reviewed 21-target scope differs')
    selections.extend([{'upstream_path':p,'decision':'defer','owner':'A29 source-witness owner','reason':reason} for p,reason in [
        ('data/phi0474/phi044/phi0474.phi044.perseus-lat1.xml','De Legibus CTS metadata exists but the retained tracking file says not migrated; no payload exists in this complete pinned tree. Follow a separately reviewed exact provider route.'),
