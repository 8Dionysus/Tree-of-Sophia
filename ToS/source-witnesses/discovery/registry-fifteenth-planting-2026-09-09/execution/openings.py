...
all_rows=rows
rows=[row for row in all_rows if "-" not in row["path"].split("_")[0]]
assert len(all_rows)==814 and len(rows)==802
...
with ThreadPoolExecutor(max_workers=4) as ex:list(ex.map(one,all_rows))
print(f'{len(all_rows)} incomplete openings captured, {len(rows)} selected individual versions; grouped files remain deferred')
