...
r=next(p for p in Path(__file__).resolve().parents if (p/'scripts/acquire_registry_sources.py').is_file());b=r/'ToS/source-witnesses/discovery/registry-fifteenth-planting-2026-09-09';e=b/'evidence';e.mkdir(parents=True,exist_ok=True)
...
if __name__=='__main__':
 ...
 for name,sha in jobs:
  x=json.loads((r/'ToS/source-witnesses/discovery/registry-fourteenth-planting-2026-09-09/evidence'/name).read_text())
  assert x['sha']==sha and not x.get('truncated')
  for row in x['tree']:
   if row['path'] in {f'sn{n}' for n in range(12,23)}:
