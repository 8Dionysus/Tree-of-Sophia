"""Ninth planting: review translation metadata before acquiring complete files."""
...
def prepare():
    ...
    prior_targets = {}
    prior_plans = {}
    old_manifest = json.loads((BASE.parent/'registry-eighth-planting-2026-09-09/manifest.corrected.json').read_text())
    ...
    for work_urn, rows in sorted(groups.items()):
        if work_urn == 'urn:cts:greekLit:tlg0007.tlg078':
            alternatives.extend({'upstream_path': row['path'], 'decision': 'defer', 'owner': 'ToS/source-witnesses/discovery', 'reason': 'Both supplied English versions have translation div xml:lang=grc, conflicting with CTS English and visible English wording. The current acquisition profile rejects this envelope-language conflict; source bytes are not normalized.', 'next_condition': 'Exact source-owner review of the conflicting language carrier and a bounded supported acquisition contract, or a corrected separately pinned provider version.'} for row in rows)
            continue
    ...
    for work_urn, row in selected:
        ...
        if work_code == 'tlg095':
            limits.append('Supplied English printing-date discrepancy remains unresolved: CTS description says 1936; TEI sourceDesc says 1939. Exact digital CTS/Git identity is fixed without adjudicating the print date.')
    ...
    if len(targets) != 44 or len({target['ids']['work'] for target in targets}) != 44:
