...
def report(completed):
    ...
    if not completed:
        ...
        write(BASE / 'local-presence.preparation.json', {'observed_at': now, 'scope': 'this checkout before ninth acquisition', 'targets': observations})
    ...
    for index, target in enumerate(manifest['targets'], 1):
        ...
        entry = {'target_id': f'registry-ninth-planting.{index:02}.{target["slug"]}', 'preferred_label': target['title'],
    ...
    result = {'schema_version': 'tos_registry_first_planting_result_v1', 'completed_at': now,
        ...
        'scope': '44 English versions of Plutarch Moralia Greek Works, preserving shared Work identity and parallel Greek reading routes.',
    ...
    lines = ['# Девятая посадка: английские переводы Moralia', '',
    ...
    for branch in sorted({plan['anchor_preparation']['branch_path'] for plan in branches.values()}):
        ...
        for path in [branch_root / 'README.md', branch_root / 'sources/README.md']:
            ...
            marker = '## Registry planting: Plutarch Moralia English (2026-09-09)'
