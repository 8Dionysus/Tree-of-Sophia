...
| 10 | tracked work/witness catalog | tracked agent, work, expression, edition, item, and collection catalogs exist; the Russian edition routes retain their official identities, while the four public Zarathustra part witnesses now resolve separately through DNB, DTA IDs `16840`–`16843`, exact SBB-PK or SUB Göttingen holding copies, and the ordered TextGrid crosscheck | proven for the bounded Nietzsche corpus | not a complete catalog of Nietzsche or philosophy; the DTA transcriptions are not a critical edition, and the four publications must not be flattened into one silent original |
...
| 14 | source discovery protocol | `source-witnesses/discovery/DISCOVERY_PROTOCOL.md` defines ordered queries and evidence capture; the eKGWB and Russian-edition runs preserve their exact results; a fourth run acquires the 1913 Commons scan; a fifth resolves DTA part 1; a sixth continues the same DNB → DTA → TextGrid order across parts 2–4, preserves the unchanged five-result TextGrid response, and acquires three exact TEIs | proven as protocol and exercised on six targets | future target searches still need exact run records; the eKGWB run selected no critical witness, the Wikisource text is not accepted, and all DTA readings remain external candidates |
...
| 16 | ID, anchor, provenance, sign, and claim contracts | `CORPUS_FOUNDATION.md`, `CORPUS_CONTRACTS.md`, and the corresponding schemas separate identity, address, occurrence, sign, and claim; the 82 witness-structure correspondences now resolve to 246 stable proposed TEI, EPUB-member, and PDF-page anchor IDs without source text | proven as contract and proposed structural addressing | the structural IDs are not exact accepted passage boundaries; real accepted signs and claims remain absent |
