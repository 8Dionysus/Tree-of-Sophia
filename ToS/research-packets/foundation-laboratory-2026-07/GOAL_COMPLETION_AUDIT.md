...
- Tree of Sophia source/contract base through `03b5f35` (`Reconcile lexical
  foundation evidence`);
...
| ---: | --- | --- | --- | --- |
| 1 | ordered research packet | `RESEARCH.md` preserves the broad foundation order; `LOCAL_LLM_ADMISSION.md` refreshes official docs, established work, then July 2026 primary work for the next software/LLM choice; `BOUNDED_TRANSLATION_SOURCE_ADMISSION_RESEARCH.md` repeats the order for the provider-transcription-to-local-calibration boundary; `HISTORICAL_GERMAN_MORPHOLOGY_ADMISSION.md` repeats it through exact 2026-07-29 release/artifact evidence for the next linguistic layer | proven and refreshed 2026-07-29 | repeat the same ordered check before any later acquisition |
| 2 | software/model decision matrix | `DECISION_MATRIX.md` covers OCR, structure, historical-German morphology, alignment, LLM, MT, embeddings, retrieval, annotation, graph, discovery, and access; the morphology refresh admits only a future sequential identity/DWDSmor/ZDL/DTAEC comparison and no current download or run | proven and refreshed 2026-07-29 | artifact-level B/C rights and dependency audit plus an exact task trigger before acquisition |
...
| 13 | rights records | item-level rights records exist and fail closed when evidence is unresolved; doctrine and discovery require positive routing for verified public-domain, open-license, permission-granted, and conditional noncommercial material; the Antonovsky 1913 and Naumann 1886 records preserve exact-item Public Domain Mark and open-route evidence without authorizing upload of the operator-held copies; all four DTA records preserve current CC BY-SA evidence, four stale live CC BY-NC fields, and separate holding-library facsimile rights without treating license evidence as textual quality or final publication authority | prepared; positive and conflicting evidence captured; human/legal review open | per-object, per-layer, jurisdiction-aware assessment and exact-term evidence before publication routing |
...
| 18 | lexical and semantic annotation packet contracts | lexical-plan/projection, semantic ladder v3, sign annotation, claim packet, graph query, historical universal v1 evidence, task-specific semantic v2, and task-specific LLM v2 contracts exist; a deterministic whole-work source observation now materializes 86,287 local exact occurrences across 506 DTA body pages and 213 divisions, with 11,352 tracked hash/count/resource rows and passing exact/normalized/prefix/phrase/section/page/language/edition probes; `HISTORICAL_GERMAN_MORPHOLOGY_ADMISSION.md` now separates surface, tokenization, conservative normalization, modernization, lemma candidate set, lemma POS, occurrence POS, morph features, compound/root analysis, provider evidence, and review while freezing an identity/DWDSmor/ZDL/DTAEC future comparison with zero installs or tasks; the source-bearing SQLite stays ignored and the tracked projection carries no strings, sequence, context, or positions; the initial sign packet remains content-free and `blocked-not-materialized`, separates occurrence/lexeme/sign/concept/claim/relation identities, and schedules zero work before its exact source question exists | proven as mechanical lexical foundation, prepared morphology method, and semantic contract; refreshed 2026-07-29 | exact source-triggered morphology question, stack profile, B/C artifact-rights audit, host preflight, and language-competence evidence before accepted lemma; one packet-local accepted German source bundle, a concrete sign candidate and frozen unassisted baseline, followed by an actual human promotion decision |
| 19 | A/B/C experiment specifications in `abyss-stack` | the foundation laboratory has declarative OCR, structure, text and visual retrieval, graph, translation, semantic, LLM, and transfer routes; the LLM route freezes resident E2B/E4B/Qwen3-4B and requires the exact ready Tree plan at preflight; its Tree gate now depends on twenty task-specific source units and their unassisted baseline rather than historical 30/15 packet completion; the Qwen3 4B card retains the April short-control evidence while explicitly blocking the exact current GPU route for translation-sized work after the i915 hang; the separate visual-retrieval route fixes the exact Qwen3-VL-Embedding-2B revision, offline CPU closure, immutable A/B controls, 20-query/36-image crosswalk, image-use and zero-network audits, and refuses execution without fresh resource admission; the newly researched historical-German morphology route is intentionally not yet a stack profile | proven for existing software/method routes; morphology experiment specification prepared only in the Tree owner | morphology still needs one exact source-triggered input packet and matching stack profile before acquisition; LLM task instances remain deliberately absent; no human work is scheduled; several content-quality experiments remain blocked or unreviewed; any GPU re-entry is a distinct source-free runtime experiment; direct-visual C still has zero runs because the latest owner gate blocked on available memory |
...
| 24 | retrieval comparison report | lexical, resident semantic/rerank, and independent semantic routes ran on the frozen small 24-passage corpus; the separate full-work DTA lexical observation index proves deterministic exact/prefix/phrase navigation but is not a rerun or quality expansion of that comparison; a distinct output-blind visual route binds the same 20 query bytes to all 36 frozen page images, exact Qwen model/runtime/control provenance, and a fail-closed visual-input audit; its latest preflight passed plan, runtime, storage, load, thermal, CPU, license, and offline gates but stopped before preparation at 11,545,407,488 available bytes versus the frozen 17,179,869,184-byte minimum and owner `force_required` | partial; full-work lexical mechanics proven, direct visual challenger resource-blocked before output | one unforced owner-admitted visual C run, plus triggered human relevance labels, hard-negative adjudication, and manual metric recomputation if an adoption-quality question is opened |
...
| 26 | quality/cost/speed matrix | machine speed, memory, swap, and storage measurements are consolidated; direct-visual one-time acquisition/runtime bytes and the blocked preflight are separated from still-unmeasured run cost | partial | visual execution speed/peak memory and human correction cost remain incomplete; the first observed candidate-review time is display-position-confounded |
...
| A/B/C results are reproducible | partial | current mechanical slices are reproducible; content-quality repeats remain incomplete |
...
5. Keep direct visual retrieval separate from the completed text A/B/C
   comparison. Its model and offline runtime are admitted, but the latest
   heavy/indexing preflight is resource-blocked. Do not force the owner or
   weaken the 16 GiB floor; execute only after a fresh `ready` receipt, and do
   not interpret setup success as page relevance or source truth.
