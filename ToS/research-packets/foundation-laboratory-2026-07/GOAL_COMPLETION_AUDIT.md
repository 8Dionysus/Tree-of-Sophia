...
| 9 | narrow `.gitignore` | only item `payload/` bytes and declared private/local lanes are ignored; payload metadata remains tracked | proven | continue checking actual `git check-ignore` and `git ls-files` behavior |
...
| 12 | checksums and forensic reports | SHA-256 fixity and forensic reports exist for all three operator-supplied files and the acquired image PDF | proven | BLAKE3 was not adopted as a second required fixity algorithm |
...
| 14 | source discovery protocol | `source-witnesses/discovery/DISCOVERY_PROTOCOL.md` defines ordered queries and evidence capture | proven as protocol | new discovery runs still need exact run records |
| 15 | access-request workflow | schemas, private/public lanes, letter template, redacted receipt template, and an unsent Wörterbuch request exist | prepared | human approval, sending, response, and permission scope |
...
| 18 | semantic annotation packet contracts | semantic ladder, sign annotation, claim packet, graph query, and source-gated evaluation contracts exist | proven as contract | real human sign decisions and accepted semantic packets |
| 19 | A/B/C experiment specifications in `abyss-stack` | the foundation laboratory has declarative OCR, structure, retrieval, graph, translation, semantic, LLM, and transfer routes; the LLM route now freezes resident E2B/E4B/Qwen3-4B and requires the exact ready Tree plan at preflight | proven as software/method and fail-closed gate | LLM task instances remain deliberately absent; several content-quality experiments remain blocked or unreviewed |
...
| source works live inside `ToS/` but outside Git | proven for four local payloads, including all three supplied files | maintain local custody and ignore checks |
