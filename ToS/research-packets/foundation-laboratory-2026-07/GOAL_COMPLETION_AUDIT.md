...
| 19 | A/B/C experiment specifications in `abyss-stack` | the foundation laboratory has declarative OCR, structure, text and visual retrieval, graph, translation, semantic, LLM, transfer, and historical-German morphology routes; the morphology profile binds the exact Tree plan and input, freezes A/B/C before output, admits only A first, builds an exact offline DWDSmor runtime, executes two whole-vocabulary passes, preserves every analysis and unknown, and records coverage, distributions, repeatability, speed, memory, bytes, and provenance; the LLM and direct-visual routes retain their separate source/resource gates | proven for the morphology A execution route and existing software/method routes; content quality remains partial or blocked | contextual morphology B/C remain unmaterialized and unacquired until the declared trigger, rights review, and fresh preflight; LLM task instances remain absent; no morphology human work is scheduled; any GPU re-entry remains a distinct source-free experiment; direct-visual C still has zero runs because the latest owner gate blocked on available memory |
...
| 26 | quality/cost/speed matrix | machine speed, memory, swap, and storage measurements are consolidated; morphology A now separates exact type/token coverage, two-pass speed, process and host-cgroup memory, zero swap, runtime/input/raw bytes, unmeasured accuracy, and zero human cost; direct-visual one-time acquisition/runtime bytes and the blocked preflight remain separated from still-unmeasured run cost | partial with morphology A mechanics measured | morphology human quality/correction cost and visual execution speed/peak memory remain incomplete; the first observed OCR candidate-review time is display-position-confounded |
