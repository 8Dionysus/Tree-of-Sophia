...
- Tree of Sophia through `dcad681` (`Admit bounded Zarathustra translation
  ...
  working tree;
- `abyss-stack` through `28e4b6b` (`Separate translation runtime lifecycle evidence`);
...
| 17 | translation packet contracts | source-review, sample, reference, pre-draft, packet, laboratory-plan, assisted-review, critical-witness, text-free German-triangulation, and bounded local research-input schemas exist; the first triangulation records exact eKGWB/DTA normalized agreement plus the Naumann OCR residual, and one DTA-derived sentence is admitted only to ignored local machine-method calibration; A is frozen contract-complete with Russian-surface defects, B is frozen invalid at the output limit, and C is frozen as a GPU runtime failure without a candidate | proven as contract and completed bounded calibration-only execution | authenticated/admitted critical witness, accepted German source, and actual independent accepted-translation lanes remain open; any runtime diagnosis is a separate experiment |
