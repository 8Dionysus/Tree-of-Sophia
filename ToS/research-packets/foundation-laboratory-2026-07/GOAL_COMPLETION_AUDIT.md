...
| 11 | manifests for the three supplied files | Antonovsky PDF, *Mysl* PDF, and Naumann EPUB have Work/Expression/Edition/Item routes and tracked manifests; two separately acquired scan items and four structured DTA TEI items add their own manifests; external identifiers remain attached to matching editions/collections/items rather than being copied by title similarity | proven | exact acquisition history and rights still carry their review posture |
| 12 | checksums and forensic reports | SHA-256 fixity and forensic reports exist for all three operator-supplied files, both acquired scan PDFs, and all four acquired DTA TEIs; the open-source byte counts, local hashes, ignore rules, and absence from tracked files were checked independently in the canonical checkout on 2026-07-28 | proven | BLAKE3 was not adopted as a second required fixity algorithm |
