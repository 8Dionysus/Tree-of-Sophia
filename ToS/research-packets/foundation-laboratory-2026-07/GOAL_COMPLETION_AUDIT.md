...
| 18 | semantic annotation packet contracts | semantic ladder v3, sign annotation, claim packet, graph query, historical universal v1 evidence, task-specific semantic v2, and task-specific LLM v2 contracts exist; the tracked initial sign packet is content-free and `blocked-not-materialized`, separates occurrence/lexeme/sign/concept/claim/relation identities, and schedules zero work before its exact source question exists; the semantic v2 plan separately freezes the A/B/C task family | proven as contract and refreshed 2026-07-29 | one packet-local accepted German source bundle, language-competence evidence, a concrete candidate and frozen unassisted baseline, followed by an actual human promotion decision |
...
| 24 | retrieval comparison report | lexical, resident semantic/rerank, and independent semantic routes ran on the frozen small corpus; a distinct output-blind visual route now binds the same 20 query bytes to all 36 frozen page images, exact Qwen model/runtime/control provenance, and a fail-closed visual-input audit; its latest preflight passed plan, runtime, storage, load, thermal, CPU, license, and offline gates but stopped before preparation at 11,545,407,488 available bytes versus the frozen 17,179,869,184-byte minimum and owner `force_required` | partial; direct visual challenger resource-blocked before output | one unforced owner-admitted visual C run, plus triggered human relevance labels, hard-negative adjudication, and manual metric recomputation if an adoption-quality question is opened |
