...
- Tree of Sophia through `91bd13f` (`Reconcile Russian edition provenance`),
  plus the open-witness admission audited in this working slice;
...
| 10 | tracked work/witness catalog | tracked agent, work, expression, edition, item, and collection catalogs exist; the Antonovsky 2007 edition is reconciled to RSL record `01005395580`, the *Mysl* 1996 volume/set to NBUV and CiNii record `BA35025034`, and the distinct Antonovsky 1913 expression/edition/item to official Wikimedia records and the acquired scan | proven for the bounded Nietzsche corpus | not a complete catalog of Nietzsche or philosophy; the title page does not itself establish Wikisource's “fourth edition” label |
| 11 | manifests for the three supplied files | Antonovsky PDF, *Mysl* PDF, and Naumann EPUB have Work/Expression/Edition/Item routes and tracked manifests; two separately acquired scan items add their own manifests; external catalog identifiers remain attached to matching editions/collections/items rather than being copied by title similarity | proven | exact acquisition history and rights still carry their review posture |
| 12 | checksums and forensic reports | SHA-256 fixity and forensic reports exist for all three operator-supplied files and both acquired scan PDFs; the Wikimedia Commons API SHA-1, downloaded byte count, local SHA-256, ignore rule, and absence from tracked files were checked independently in the canonical checkout on 2026-07-28 | proven | BLAKE3 was not adopted as a second required fixity algorithm |
| 13 | rights records | item-level rights records exist and fail closed when evidence is unresolved; doctrine and discovery require positive routing for verified public-domain, open-license, permission-granted, and conditional noncommercial material; the Antonovsky 1913 record preserves Commons' exact-object public-domain declaration and the distinct Wikisource CC BY-SA candidate without treating either as final authorization or quality proof | prepared; positive evidence captured; human/legal review open | per-object, per-layer, jurisdiction-aware assessment and exact-term evidence before publication routing |
| 14 | source discovery protocol | `source-witnesses/discovery/DISCOVERY_PROTOCOL.md` defines ordered queries and evidence capture; the exact eKGWB run preserves GND identity, direct-source failure, query error, and noisy fallback order; two runs reconcile the supplied Russian editions through national and academic catalogs; a fourth run directly re-queries official Commons and Wikisource endpoints, acquires the exact 1913 scan, and defers the incompletely proofread transcription | proven as protocol and exercised on four targets | future target searches still need their own exact run records; the eKGWB run selected no critical witness and the Wikisource text is not accepted |
...
| 23 | translation laboratory report | `TRANSLATION_LABORATORY_REPORT.md` records source-gate and interface preparation | prepared, not executed for quality | accepted German source, independent drafts, reference reveal, etymology, and adjudication |
...
| 30 | future server-import contract | schemas, protocol, item plans, rights gate, takedown fields, and metadata-only status exist; the operator policy keeps present local payloads off the future public site while permitting public-safe source identity and provenance; the 1913 plan preserves positive source rights evidence but still authorizes no payload transfer | proven as blocked contract and payload non-publication policy | separate reviewed publication decision and operator approval for any material that may actually be published |
...
| source works live inside `ToS/` but outside Git | proven for five local payloads, including all three supplied files; canonical payload fixity/ignore/tracked-file checks passed on 2026-07-28 | maintain local custody and ignore checks |
