...
| 10 | tracked work/witness catalog | tracked agent, work, expression, edition, item, and collection catalogs exist; the Antonovsky 2007 edition is reconciled to RSL record `01005395580`, while the *Mysl* 1996 volume/set is reconciled to NBUV and CiNii record `BA35025034` | proven for the bounded Nietzsche corpus | not a complete catalog of Nietzsche or philosophy |
| 11 | manifests for the three supplied files | Antonovsky PDF, *Mysl* PDF, and Naumann EPUB have Work/Expression/Edition/Item routes and tracked manifests; external catalog identifiers remain attached to editions/collections rather than being copied onto unmatched digital items | proven | exact acquisition history and rights still carry their review posture |
| 12 | checksums and forensic reports | SHA-256 fixity and forensic reports exist for all three operator-supplied files and the acquired image PDF; the 2026-07-28 canonical-checkout `--require-local-payloads` validation matched every required local payload to its tracked fixity | proven | BLAKE3 was not adopted as a second required fixity algorithm |
| 13 | rights records | item-level rights records exist and fail closed when evidence is unresolved; doctrine and discovery now require positive routing for verified public-domain, open-license, permission-granted, and conditional noncommercial material | prepared; human/legal review open | per-object, per-layer, jurisdiction-aware assessment and exact-term evidence before either public or restricted routing |
| 14 | source discovery protocol | `source-witnesses/discovery/DISCOVERY_PROTOCOL.md` defines ordered queries and evidence capture; the exact eKGWB run preserves GND identity, direct-source failure, query error, and noisy fallback order; two additional protocol-native runs reconcile the supplied Russian editions through national and academic catalogs without source download | proven as protocol and exercised on three targets | future target searches still need their own exact run records; the eKGWB run selected no critical witness |
...
| 30 | future server-import contract | schemas, protocol, item plans, rights gate, takedown fields, and metadata-only status exist; the operator policy now keeps present local payloads off the future public site while permitting public-safe source identity and provenance | proven as blocked contract and payload non-publication policy | separate permission and operator approval for any material that may actually be published |
...
| source works live inside `ToS/` but outside Git | proven for four local payloads, including all three supplied files; canonical required-payload fixity validation and the tracked-file check passed on 2026-07-28 | maintain local custody and ignore checks |
