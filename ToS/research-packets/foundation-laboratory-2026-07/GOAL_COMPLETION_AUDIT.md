...
| 10 | tracked work/witness catalog | tracked agent, work, expression, edition, item, and collection catalogs exist | proven for the bounded Nietzsche corpus | not a complete catalog of Nietzsche or philosophy |
| 11 | manifests for the three supplied files | Antonovsky PDF, *Mysl* PDF, and Naumann EPUB have Work/Expression/Edition/Item routes and tracked manifests | proven | bibliographic assertions and rights still carry their review posture |
...
| 13 | rights records | item-level rights records exist and fail closed | prepared; human/legal review open | jurisdiction-complete assessment and any redistribution permission |
| 14 | source discovery protocol | `source-witnesses/discovery/DISCOVERY_PROTOCOL.md` defines ordered queries and evidence capture; the first exact eKGWB run preserves GND identity, direct-source failure, query error, and noisy fallback order | proven as protocol and exercised once | future target searches still need their own exact run records; the first run selected no critical witness |
...
| 21 | OCR comparison report | `OCR_ADMISSION.md` records mechanical A/B/C execution and negative evidence; one later private method-blind candidate-review pass exists | partial | no independent gold, CER/WER, balanced correction-cost comparison, or general winner |
...
| automatic metrics are manually rechecked | open | recompute at least one metric from raw artifacts and accepted human evidence |
