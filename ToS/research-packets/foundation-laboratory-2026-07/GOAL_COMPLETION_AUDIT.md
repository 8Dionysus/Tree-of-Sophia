...
- Tree of Sophia through `ddcc0dd` (`Map Zarathustra witness structure`),
  plus the stable proposed structural-address layer audited in this working
  slice;
...
| 10 | tracked work/witness catalog | tracked agent, work, expression, edition, item, and collection catalogs exist; the *Mysl* volume now resolves seven distinct works and seven provisional Russian expressions, with six newly introduced whole-work identities grounded by current DNB/GND records; the four public Zarathustra part witnesses remain separately resolved through DNB, DTA IDs `16840`–`16843`, exact SBB-PK or SUB Göttingen holding copies, and the ordered TextGrid crosscheck | proven for the bounded Nietzsche corpus | not a complete catalog of Nietzsche or philosophy; the new translation-responsibility claims remain model-made and unreviewed, the DTA transcriptions are not a critical edition, and distinct publications must not be flattened |
| 11 | manifests and page/resource inventories for the source files | Antonovsky PDF, *Mysl* PDF, and Naumann EPUB have Work/Expression/Edition/Item routes and tracked manifests; two separately acquired scan items and four structured DTA TEI items add their own manifests; all nine items now carry deterministic text-free resource inventories: 2,194 PDF pages, 541 EPUB members, and 762 TEI page-break/division resources resolve to exact file digests without copying source text into Git | proven mechanically | exact acquisition history and rights still carry their review posture; resource enumeration is not content acceptance |
| 12 | checksums and forensic reports | SHA-256 fixity and forensic reports exist for all three operator-supplied files, both acquired scan PDFs, and all four acquired DTA TEIs; the open-source byte counts, local hashes, ignore rules, and absence from tracked files were checked independently in the canonical checkout on 2026-07-28; each resource inventory has its own digest-bound provenance event | proven | BLAKE3 was not adopted as a second required fixity algorithm |
| 13 | rights records | item-level rights records exist and fail closed when evidence is unresolved; doctrine and discovery require positive routing for verified public-domain, open-license, permission-granted, and conditional noncommercial material; the Antonovsky 1913 record preserves Commons and Wikisource evidence; all four DTA records preserve current CC BY-SA evidence, four stale live CC BY-NC fields, and separate holding-library facsimile rights without treating license evidence as textual quality or final publication authority | prepared; positive and conflicting evidence captured; human/legal review open | per-object, per-layer, jurisdiction-aware assessment and exact-term evidence before publication routing |
| 14 | source discovery protocol | `source-witnesses/discovery/DISCOVERY_PROTOCOL.md` defines ordered queries and evidence capture; the eKGWB and Russian-edition runs preserve their exact results; the open-witness runs acquire the 1913 Commons scan and four DTA parts; a seventh run resolves the six previously absent whole works in the *Mysl* volume through DNB/GND and explicitly rejects two section-level records plus one unrelated result | proven as protocol and exercised on seven targets | future target searches still need exact run records; the eKGWB run selected no critical witness, the Wikisource text is not accepted, and all DTA readings remain external candidates |
...
| 30 | future server-import contract | schemas, protocol, nine item plans, rights gate, takedown fields, and metadata-only status exist; the operator policy keeps present local payloads off the future public site while permitting public-safe source identity and provenance; the 1913 and four DTA plans preserve positive source-rights routes and unresolved layer-specific evidence while authorizing no payload transfer | proven as blocked contract and payload non-publication policy | separate reviewed publication decision and operator approval for any material that may actually be published |
...
| source works live inside `ToS/` but outside Git | proven for nine local payloads, including all three supplied files; canonical payload fixity/ignore/tracked-file checks passed on 2026-07-28 | maintain local custody and ignore checks |
| manifests, fixity, rights, and provenance are tracked | proven structurally | complete human rights review where required |
| every source file has a page/resource inventory | proven mechanically for all nine local payloads | retain the text-free boundary and rebuild only from exact fixity-verified bytes |
