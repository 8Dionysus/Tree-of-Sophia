...
- Tree of Sophia through `24e9a8a` (`Record Qwen GPU hang diagnosis
  boundary`), plus the task-specific semantic-assurance refresh in this
  working tree;
- `abyss-stack` through `fed26c8` (`Record Qwen GPU stop-line and close test
  inventories`);
...
| 19 | A/B/C experiment specifications in `abyss-stack` | the foundation laboratory has declarative OCR, structure, retrieval, graph, translation, semantic, LLM, and transfer routes; the LLM route freezes resident E2B/E4B/Qwen3-4B and requires the exact ready Tree plan at preflight; its Tree gate now depends on twenty task-specific source units and their unassisted baseline rather than historical 30/15 packet completion; the Qwen3 4B card now retains the April short-control evidence while explicitly blocking the exact current GPU route for translation-sized work after the i915 hang | proven as software/method and fail-closed gate | LLM task instances remain deliberately absent; no human work is scheduled; several content-quality experiments remain blocked or unreviewed; any GPU re-entry is a distinct source-free runtime experiment |
...
| heavy models/caches use host-owned storage | proven for current laboratory acquisitions | rerun host admission before every new candidate |
