...
| specialized MT setup / MADLAD-400 3B q4 + Candle CPU | exact three-file model set, locked offline runtime, and one invented source-free DE→RU smoke; private Nietzsche source runs 0 | source-free output manually read as `Окно открыто.` for mechanical sanity only; source/private German fidelity, literary quality, Russian floor, and human review absent | successful acquisition service 431.423 s; fresh runtime-build service 432.966 s; smoke owner service 13.377 s and complete caller wall 264.620562 s; model load 6.293957121 s; generation 4.477793941 s for 5 tokens | 1,671,227,060 model bytes; 9,667,291 admitted runtime bytes; acquisition peak 1,653.359 MiB RAM + 0.523 MiB swap; build peak 1,852.047 MiB RAM + 29.023 MiB swap; smoke peak 3,486.566 MiB + zero swap; redundant-preflight, compile/API, invalid-environment-manifest, and schema-boundary-repair negatives retained; one `0644` worker output corrected to `0600` without byte change and future closure verifier passes | zero; no human task opened | retain exact setup and failures; private candidate remains not run; no model winner, quality, translation, German, etymology, semantic, graph, publication, or canon claim |
