...
| native structure A, corrected | 36 units | all anchors resolved; 12 outline-PDF pages falsely empty; inherited OCR defects; same-page comparison gives no visual-structure preference but preserves cheap native-layer evidence; human structure receipts 0 | 1.228 s | 32,632,832 peak RSS; 172,489 derived bytes | not measured | retain with limits as inventory/native-layer companion; no accepted text |
| structure B / Docling hybrid R2 | 36 units; 12 per exact provenance-routed branch | six advisory inspections: 1 accept-with-limits, 5 reject; later same-page comparison boundedly prefers B on Antonovsky 330; mechanical anchor resolution 1.0; structural quality not computable; human structure receipts 0 | 73.305 s runner; 29.466 units/min; 90.459 s owner wall | 1,582,149,632-byte child peak RSS; 3,250.324 MiB owner peak, zero swap; 1,267,021 artifact bytes | not measured | fast hybrid comparator and bounded prose-order route; outputs unaccepted; no routine human task |
| structure C / PaddleOCR-VL full R1 | frozen 12-page visual subset | six advisory inspections: 5 accept-with-limits, 1 reject; later same-page comparison boundedly prefers C on 5/6 pages but preserves its Antonovsky 330 regression; 164 blocks, zero empty pages; preference count is not structural F1 or reading-order accuracy; human structure receipts 0 | 5,672.930 s runner; 0.1269 pages/min; 5,687.198 s owner wall; page predictions 22.523–834.815 s | 9,674,465,280-byte child peak RSS; owner peak 10,694.297 MiB RAM + 1,005.258 MiB swap; 515,122 artifact bytes | not measured | high-cost question-scoped visual challenger; no universal winner or routine human task |
...
| source-anchor v2 synthetic A/B/C | three invented public representations; three exact selections; six negative controls | 3/3 selections resolve and match independent `jq`/`rg`/Unicode/`xmllint` inspection; 6/6 malformed or unsafe controls rejected; human reviews 0; no real source or accepted text | five warm separate CLI processes: 0.19–0.20 s wall each | 47,268–47,464 KiB max RSS; tracked synthetic fixtures only; no model, network, GPU, private source, or new runtime | zero direct charge; electricity and fully loaded cost not measured; no human task opened | promote additive address mechanics with limits; keep v1 unchanged; no bulk migration, source authority, review, semantics, or canon |
| source-text-layer synthetic A/B/C | three invented public UTF-8 representations; two explicit code-point transitions; nine negative controls | exact source-anchor return, predecessor/content digests, A→B insertion, B→C replacement, and independent `NFC(B) == C` code-point inspection agree; 9/9 silent, drifting, overclaiming, leaking, or self-referential controls rejected; human reviews 0; no real source or accepted text | one observed focused validator process: 0.25 s wall; not a benchmark | 49,176 KiB max RSS in that run; tracked synthetic bytes only; no model, network, GPU, private source, or new runtime | zero direct charge; electricity and fully loaded cost not measured; no human task opened | promote immutable-layer, exact-derivation, and separated-gate mechanics with limits; no corpus migration, accepted transcription, competence, downstream use, publication, semantics, or canon |
| provenance-event v2 synthetic A/B/C | three separate actual commands over one invented public NFD input fixture: exact copy, NFC transform, and intentional exit-7 ASCII-strict failure; fourteen negative controls | exact event/entity/argv/derivation/manifest closure; A bytes equal input, B bytes equal independent NFC, C has zero output and one exact failure byproduct; 14/14 drift or authority-overclaim controls rejected; execution truth and content quality remain unmeasured; human reviews 0 | one observed focused validator process: 0.26 s wall; command execution itself not benchmarked | 50,424 KiB max RSS in that validator run; tracked synthetic bytes only; no model, network, GPU, private source, or new runtime | zero direct charge; electricity and fully loaded cost not measured; no human task opened | promote immutable receipt and honest success/failure/byproduct mechanics with limits; unsigned fixity is not authentication or execution truth; no v1 migration, accepted content, rights, publication, semantics, graph, or canon |
...
| historical-German contextual morphology B | one exact-form A ambiguity; output-blind recurrence ranks 1/73/145; two raw-TEI paragraphs and one verse group across parts 1/3/4; exact private contexts frozen before B output | retained 41-wheel negative admission plus later 42-wheel five-control `local_dev` admission; all 3 targets align to one token and receive provider `ADV`/`ADV` proposals; both streams byte-identical; German gold, contextual POS accuracy, accepted tokenization, morphology, lemma, and lexeme remain 0 | corrected source-free runtime build 15.889 s owner service; B pass 1 wall/CPU 0.098123434/0.088108849 s, pass 2 0.084551051/0.084010459 s, 30.573736 contexts/s; execution service 11.118 s wall / 9.935 s CPU | 5,203-byte private packet; 627,548,130-byte principal wheel plus 41 dependency wheels; 44-file subject store; 977,683,987-byte runtime; 39,869-byte private output; process peak RSS 1,273,647,104 bytes; execution owner footprint peak 1,892.715 MiB and swap peak 0.25 MiB | model quality and human correction not measured; no human task opened; artifact/build/execution costs remain separate | retain both negative and positive evidence; promote only executable private method mechanics and text-free receipt; labels remain proposals, redistribution/public serving remain blocked, production trust-root/lifecycle proof is separate, and review opens only for a later real German-competent decision |
...
| direct visual retrieval C / Qwen3-VL-Embedding-2B CPU | frozen 20 text queries × 36 page images; one audit-complete r9 run plus preserved audit-incomplete r8 | 20/20 repeat-stable rankings; 200/200 anchors resolve; advisory target top-10 19/19 evaluable; exact five-query source-visible model inspection confirms one A/B-missed image-only route recovery and four rank-1 language/witness-policy failures; human judgments 0 | 1.042 s model load; 2,507.904 s page encoding; 6,800.709 ms first and 6,955.081 ms warm median query; 2,806.762 s runner / 2,807.234 s owner wall | 4,742,041,600-byte bridge peak RSS; 4,808,843,264-byte owner memory peak + 380,157,952-byte swap peak; 1,588,701-byte vector index; one-time 4,549,670,315-byte acquisition + 1,182,279,019-byte runtime; 76 persisted normalization records; r8/r9 rankings and scores identical 20/20 | not measured; advisory inspection is not human time; human debt 0 | reject current unfiltered C as default; retain r9 only as opt-in post-text-coverage source-page fallback with predeclared item/witness/language/content-class constraints; no relevance winner, promotion, or production/public adoption |
...
| semantic identity/annotation v2 public-synthetic A/B/C | one invented 109-byte UTF-8 source, two exact occurrences, source-only A, reciprocal competing-proposal B, deliberately invalid-promotion C, and twelve in-memory negative controls; no real model/software/human act | exact anchors and digests independently resolve; A and B validate, C fails schema and semantic closure, and all twelve controls reject; semantic correctness, agreement, promotion quality, and real-source transfer remain unmeasured | deterministic builder/validator execution completed but no benchmark protocol was declared, so latency and throughput are not comparable evidence | existing Python/JSON Schema route only; no model, download, runtime, or source payload | zero; no human task opened | promote contract and fail-closed mechanics with limits; no sign, relation, concept, graph, canon, model, or human-quality conclusion |
| translation-alignment v1 public-synthetic A/B/C | two invented UTF-8 sides under `x-tos-src`/`x-tos-tgt`, frozen synthetic segmentation/tokenization, exact anchors, one-to-one A, reciprocal one-to-one/one-to-many B, invalid-acceptance C, and seventeen in-memory controls; no translation/model/aligner/human act | both side bytes, digests, analysis bindings, and anchor selections resolve; A and B validate, C fails schema and semantic closure, and all seventeen controls reject; real alignment accuracy, literary adequacy, competence, and transfer remain unmeasured | deterministic builder/validator execution completed but no benchmark protocol was declared, so latency and throughput are not comparable evidence | existing Python/JSON Schema route only; no model, download, runtime, or corpus payload | zero; no human task opened | promote additive identity/provenance and fail-closed mechanics with limits; no real alignment, translation quality, lexical equivalence, export authority, graph, canon, method winner, or human-quality conclusion |
...
1. only the smallest predeclared source-visible subset needed by a concrete
   OCR/structure adoption or promotion question; never an automatic 15-page
   human batch merely because candidates exist;
...
3. 10 random and 10 hard blind human judgments for a text retrieval or graph
   quality/adoption comparison; the completed five-query direct-visual
   advisory inspection is sufficient only for its bounded fallback-routing
   decision and does not create a broad standing human batch;
