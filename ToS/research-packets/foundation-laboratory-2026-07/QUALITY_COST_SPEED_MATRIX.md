...
| historical-German contextual morphology B | one exact-form A ambiguity; output-blind recurrence ranks 1/73/145; two raw-TEI paragraphs and one verse group across parts 1/3/4; exact private contexts frozen before B output | packet and exact-return mechanics proven; B output, German gold, contextual POS quality, accepted morphology, lemma, and lexeme remain absent | packet preparation only; B execution not measured | 5,203-byte ignored mode-0600 context packet; B's exact 627,548,130-byte `de_zdl_lg 4.0.0` wheel identified but unacquired; dependency closure and runtime/peak evidence absent | not measured; no human task opened | admit B-only execution preparation after artifact-rights and host gates; preserve A ambiguity rather than manufacture a verdict |
