...
| historical-German morphology A/B/C | identity control plus exact DWDSmor Open 0.18.0, ZDL `de_zdl_lg 4.0.0`, and DTAEC-normalized ZDL routes researched; 0 acquired artifacts, profiles, tasks, or runs | no corpus output, German gold, accepted lemma, or lexeme; model-card/treebank scores deliberately not imported as ToS quality | not measured | research freezes A's 4,803,531-byte wheel, B's 627,548,130-byte package, and C's 31,741,808-byte weights; actual runtime/install/peak evidence absent | not measured; no human task opened | preparation-only admission; acquire sequentially only after an exact morphology question, stack profile, rights review, and host preflight |
