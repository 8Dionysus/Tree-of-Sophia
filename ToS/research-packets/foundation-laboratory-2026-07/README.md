# Foundation Laboratory Research Packet
...
Mysl and Antonovsky translator-Agent identity reconciliation, exact
...
2026-08-11; immutable source-text layers, correction provenance, explicit
normalization, and their public-synthetic real-byte A/B/C replay were refreshed
the same day; immutable execution receipts, exact failure/byproduct posture,
replay/authentication/authority separation, and their public-synthetic actual-
command A/B/C were also refreshed the same day; existing v1 anchors,
provenance events, and corpus text records were not migrated

Translation-alignment identity, exact two-sided return, competing mapping
claims, real-human competence gates, visibility inheritance, and derived
interchange posture were refreshed 2026-08-11 through a separate
...
laboratory cycle around *Thus Spoke Zarathustra*. It preserves the required
research order:

1. classical standards and official documentation;
2. leading papers and established tools;
3. the freshest relevant papers and current official software surfaces.

The packet does not authorize mass ingestion, model training, unattended
semantic extraction, or promotion into canon. It narrows what should be built
and what must be tested manually.
...
| `SOURCE_TEXT_LAYER_AND_CORRECTION_PROVENANCE_RESEARCH_2026-08-11.md` | Why OCR, diplomatic transcription, reviewed reading, and normalization must be immutable successor layers; what official standards, established digital-edition practice, and fresh 2025–2026 correction-provenance evidence require; what the public-synthetic real-byte A/B/C replays; and why it creates no accepted text, competence, semantics, publication, or migration? |
...
| `MYSL_1996_VOLUME_2_LAYERED_RIGHTS_ASSESSMENT.md` | Why the exact local 1996 aggregate must be assessed as eleven distinct RU/US layers; why Nietzsche, Antonovsky, and Polilov historical substrates do not clear Svasyan's translation, editing, compilation, notes, the unresolved Flerova, publisher, portrait, scan, or ABBYY layers; why public discoverability is not permission; and why the Item and every content-bearing server derivative remain closed? |
...
| `ANTONOVSKY_1913_LAYERED_RIGHTS_ASSESSMENT.md` | Why the exact fixity-matched Commons object supports positive RU/US public-domain assessments for the work, translation, preface, historical presentation, faithful scan, and mechanical ABBYY layer; why §104A(h)(8)(C)(i) makes Germany and its 1970 endpoint, not Russia's separate 1950 reviewed-jurisdiction result, the source-country operand for Nietzsche's published original Work; and why text quality, other jurisdictions, human review, operator approval, local-payload transfer, and publication remain independently closed? |
| `DTA_ZARATHUSTRA_PARTS_1_4_LAYERED_RIGHTS_ASSESSMENT.md` | Why the four exact annotated TEIs are positively CC BY-SA 4.0; why their independent US historical route follows §104A's source-country-term condition rather than a domestic pre-1931 shortcut; and why their Work/Edition, metadata, distinct plain-text, stale OAI, facsimile, German-quality, local-custody, and future-server layers remain separately assessed? |
| `NAUMANN_1893_IA_PDF_EPUB_LAYERED_RIGHTS_ASSESSMENT.md` | Why the exact image-container PDF can receive a positive DE/US public-domain assessment while the related automatic EPUB remains aggregate `conflicting_evidence` because it contains later unlicensed born-digital material; why the independent US historical route is §104A's source-country condition rather than a domestic pre-1931 shortcut; why the current Internet Archive terms shell does not falsely reconfirm the substantive earlier capture; and why neither local file becomes publishable or transferable? |
...
| `ANTICHRIST_1906_COMMONS_STANFORD_LAYERED_RIGHTS_ASSESSMENT.md` | Why the exact Commons/Stanford DjVu has public-domain-reviewed represented Work, historical-presentation, faithful-scan, and automatic-text layers plus separately licensed Commons metadata surfaces; why the restored-work source-country-expiration condition is §104A(h)(6)(B), while §104A(g) governs Presidential proclamation; why the aggregate remains `conflicting_evidence` because archive editorial responsibility, Stanford furniture, OCR layout, the DjVu package, and contaminated Internet Archive metadata are not completely resolved; and why its model-made pages 228-329 member boundary stays outside rights and text acceptance? |
| `MDZ_FALL_WAGNER_1888_GOETZEN_1889_LAYERED_RIGHTS_ASSESSMENT.md` | Why two exact MDZ objects with the same public-domain historical soil require different exact-object outcomes: `permission_granted` under the non-license `NoC-NC 1.0` statement for *Der Fall Wagner* and `licensed` under CC BY-NC-SA 4.0 for *Götzen-Dämmerung*; why the independent US historical route is §104A's source-country condition rather than a domestic pre-1931 shortcut; why current response-byte drift does not erase stable exact identity, rights, canvas counts, or attribution; and why separate CC0 metadata, no invented OCR layer, and operator-local/non-transfer gates remain unchanged? |
...
| `ANTONOVSKY_2007_CULTURAL_REVOLUTION_LAYERED_RIGHTS_ASSESSMENT.md` | Why Nietzsche's Work and Antonovsky's historical translation substrate are public-domain-reviewed for RU/US while the exact 2007 Expression and operator PDF remain in copyright because of separately claimed translation editing, commentary translation, and design; why the underlying commentary and publisher contribution stay unresolved; and why no local payload or content-bearing derivative becomes transferable or publishable? |
...
| `ANTONOVSKY_1911_RSL_RUNEB_LAYERED_RIGHTS_ASSESSMENT.md` | Why Nietzsche's Work, Antonovsky's 1911 translation and preface, and the historical presentation are public-domain-reviewed for RU/US while the exact RSL/RuNEB scan and embedded text remain copyright-undetermined without an object-specific reuse statement; why §104A(h)(8)(C)(i) separates Germany's 1970 original-Work source-country endpoint from Russia's 1950 reviewed-jurisdiction result; and why the aggregate file stays local-only and non-transferable? |
...
| `ENERGY_MEASUREMENT_ADMISSION_2026-08-11.md` | Which direct energy counters the current host exposes, why present counters are still unreadable to the laboratory identity, how official → established → fresh research shapes the fail-closed method, and why existing run energy remains `null` rather than zero? |
| `MONETARY_COST_ACCOUNTING_2026-08-11.md` | Why local execution is not synonymous with zero cost; how direct billed, electricity, hardware allocation, human time, and human valuation remain distinct; how v2 binds every component and attempt to byte-verified SHA-256 evidence without claiming signature authenticity; what official → established → fresh evidence supports the method; and why existing full monetary cost stays null? |
...
| `HISTORICAL_GERMAN_MORPHOLOGY_B_ARTIFACT_ADMISSION_2026-08-10.md` | Why the frozen B research packet remains immutable; what the exact private ZDL acquisition cost establishes; why the historical artifact trust gate denied runtime admission; and why no runtime, B output, linguistic result, or downstream authority exists? |
| `HISTORICAL_GERMAN_MORPHOLOGY_B_EXECUTABLE_STOP_LINE_2026-08-12.md` | Why the laboratory builder is the pre-build `agent` consumer rather than a production runtime; how reviewed `abyss-stack` checkpoint `87d85ad1` makes the exact artifact gate and retained receipt recheck executable; what the real denied invocation and full tests prove; and why rights, B output, linguistic truth, semantics, and production admission remain closed? |
| `HISTORICAL_GERMAN_MORPHOLOGY_B_EXECUTION_2026-08-12.md` | How the retained negative attempt, source-free import failure, corrected 42-wheel closure, five-control local admission, exact private two-pass B run, text-free Tree return, cost measurements, and zero linguistic/semantic authority coexist without rewriting prior evidence? |
...
| `LIVED_WITNESS_FOUNDATION_RESEARCH.md` | Why the human root needs a first-person authored record rather than TEI witness or generic semantic annotation; how raw voice, experience/capture time, AI transformations, relational privacy, separate permissions, revision, and withdrawal remain visible; and why the prepared route captures no testimony or human debt? |
...
| `GERMAN_EDITION_READING_ADMISSION_RESEARCH_2026-08-10.md` | Why an exact documented scholarly transcription may establish what one Edition reads without asserting German linguistic correctness, author-finality, accepted translation, or semantics; and how `edition_reading_attested` should open source-observational stages while later competence and human-promotion gates stay closed? |
| `AUTHORED_CANON_EVIDENCE_BRIDGE_RESEARCH_2026-08-12.md` | How the living twelve-segment Zarathustra authored route can return to the exact DTA section through two reciprocal segmentations and digest-bound inventories without deleting the seed or converting route-local witness roles, legacy reviews, 92 nodes, or 125 relations into accepted source/translation, modern human evidence, semantic or graph truth, canon revision, publication, or transfer authority? |
...
| `STRUCTURE_RECOVERY_REPORT.md` | What did the real Structure A/B/C runs produce; how do later text-free source/target numbered-label maps yield stable addresses; why 35 conservative routes become 32 intersecting target candidates plus 3 negative intersections; how the historical German pass stopped at 32 candidates plus 3 unresolved boundaries; and why none establishes accepted text or alignment? |
| `structure-common-page-comparison.v1.json` | What does a source-visible, variant-visible, text-free comparison of the same six frozen pages say about per-page A/B/C structure routing; why C is boundedly preferred on five pages and B on one; and why that count is neither a quality score, a blind review, a human task, nor content promotion? |
| `TRANSFER_SOURCE_BOUNDARY_FORENSIC_RETURN.md` | How did the later source-visible forensic return close the three retained German boundary gaps, materialize all 35 private source candidates, and derive a text-free 35-route readiness projection while preserving zero accepted text, alignment, eligibility, gold, human, semantic, publication, or canon authority? |
| `RETRIEVAL_COMPARISON_REPORT.md` | What do the executed text A/B/C routes and the separate audit-complete direct-page-image challenger prove, disagree about, cost, and how is the current visual route bounded after its exact five-query advisory inspection? |
| `direct-visual-retrieval-routing-inspection.v1.json` | Why does source-visible inspection reject the current unfiltered visual challenger as a default route, retain one opt-in post-text-coverage fallback, require predeclared item/witness/language/content-class constraints, and create no human gold, alignment, semantic authority, or backlog? |
...
| `STANFORD_LOEB_TINSLEY_PRE_STREET_DRIFT_CHECK_2026-08-10.md` | What changed before the anticipated street date: why the official catalog remains at 424 pages while current Google, Harvard, iMusic, and Stanford search-index metadata converge or drift toward 552; why preorder, an internally conflicting vendor release label, previews, excerpts, and a third-party upload do not prove an exact released manifestation or authorize content use? |
| `STANFORD_LOEB_TINSLEY_POST_STREET_METADATA_CHECK_2026-08-11.md` | What changed after the anticipated date began at Stanford: why Harvard's cart signal and the dated eBook metadata still do not resolve blocked publisher transport, the unchanged 424-page official catalog, contradictory 424/552 print feeds, exact manifestation, custody, rights, or suitability; and why an unsolicited search-result snippet is excluded from blind adjudication rather than silently ignored? |
...
| `FALL_WAGNER_AUTHORIAL_WITNESS_ROUTE.md` | How do exact W II 6/W II 7 regions, two lost complete manuscript states, the sole surviving Basel correction leaf, the author-published 1888 run, its unlabelled and nominal `Zweite Auflage` halves, and critical addresses remain distinct; why D 21 belongs to *Götzen-Dämmerung*; and why current open routes create neither reconstructed text nor transfer eligibility? |
| `GOETZEN_DAEMMERUNG_AUTHORIAL_WITNESS_ROUTE.md` | How do exact regions in mixed W II preparatory soil, D 21, late additions and section migration, the 1888-produced/1889-dated first publication, the changed 1893 edition, and critical addresses remain distinct; why *Magnum in parvo* is an editorial reconstruction challenger rather than an author-final Work; and why current open routes create neither admitted text nor transfer eligibility? |
...
| `QUALITY_COST_SPEED_MATRIX.md` | Which mechanical speed/resource values exist, which are comparable, where direct energy observation is permission-blocked, why cash-only evidence is not total monetary cost, and where quality and human correction cost remain unmeasured? |
| `GOLDEN_KERNEL_TRANSFER_REPORT.md` | Is cross-work A/B/C epistemically runnable, what do the three title-page scouts and twenty private pre-output page candidates prove, why 35 conservative routes become 32 intersecting layer-exact target candidates plus 3 explicit nonintersection negatives, and what evidence gate still blocks a false transfer claim? |
| `TEXTGRID_TRANSFER_SOURCE_CHALLENGER_REFRESH_2026-08-09.md` | Why current public TextGrid/Kolimo+ Nietzsche objects remain deferred normalized-corpus challengers rather than exact transfer witnesses; how exact revision fixity, source-edition mismatch, silent normalization, malformed metadata, and aggregate-versus-transcription rights conflict were tested; and what narrow future discrepancy question could reopen them? |
| `JENSEITS_187_TRANSFER_SOURCE_VISIBLE_REVIEW.md` | How one exact §187 route progresses from automatic source/target slices to a private model-source-visible and critical-layer triangulated candidate; which whole-line target omission and German OCR defects direct page inspection catches despite green mechanics; and why accepted text, alignment, eligibility, gold, human debt, semantics, and publication remain zero? |
...
Tree of Sophia should not begin by declaring a universal immutable inventory
of philosophical entities. Its durable foundation is an **evidence spine**:

```text
work
  -> expression / language state
    -> edition or manifestation
      -> physical or digital item
        -> immutable acquisition event and digest
          -> addressable passage or region
            -> observable occurrence
              -> versioned proposal or claim
                -> human review decision
                  -> replaceable search and graph projections
```

The bytes, acquisition record, digest, persistent identity, and review event
can be stable. A word occurrence can be stable when its witness and address
are stable. Lemmatization, sign boundaries, etymology, semantic sense,
concepts, abstractions, relations, and cross-work equivalence remain
versioned interpretations even when they become highly trusted.

This is a stronger foundation because it preserves the route back to what was
actually seen while allowing understanding to deepen.

## Laboratory truth boundary

- A validator may prove that required fields, files, and references exist.
- A benchmark may measure a bounded task on a declared sample.
- Neither proves that OCR is faithful, a translation carries the sentence, a
  semantic boundary is sound, or a graph relation is philosophically true.
- Manual comparison against visible source regions and a deliberately small
  gold set is the promotion authority for this laboratory.
- Failed, ambiguous, and rejected outputs are retained as evidence rather
  than removed to make a metric greener.

## Refresh law

The standards layer is comparatively stable. Software releases, model cards,
licenses, security notices, and 2026 research are time-sensitive. Every
future trial must record the exact version, model digest, license snapshot,
runtime, prompt or configuration, and date; a current label in this packet is
not a permanent endorsement.
