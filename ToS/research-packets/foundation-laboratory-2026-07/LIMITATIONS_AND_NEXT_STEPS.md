...
| historical-German morphology A/B/C | ordered official -> established -> current research; exact whole-vocabulary input; reproducible `abyss-stack` A profile; DWDSmor Open 0.18.0 A runtime and retained private result; one output-blind three-context ZDL `de_zdl_lg 4.0.0` B episode frozen for a concrete A ambiguity; rights-gated DTAEC-assisted C retained only for a distinct normalization question | A: 11,352 exact form types / 86,287 token weight processed twice with byte-identical streams; 6,610 types and 75,872 token weight covered; 4,742 types / 10,415 weight mechanically unknown; B: private exact-return contexts frozen at recurrence ranks 1/73/145 across parts 1/3/4; exact 41-wheel closure privately acquired; reviewed `87d85ad1` enforces the exact `agent` gate before acquisition verification/runtime/source, the real invocation fails closed, and runtime/output remain absent; C: question-inapplicable and unacquired; accuracy, competent gold, accepted morphology, lemmas, and lexemes 0; human debt 0 | coverage and unreviewed mechanical residue are not quality or a review trigger; preserve B's frozen question and negative admission evidence until explicit rights review/private-only handling, required signature, latest eligible promotion, full verification, all five controls, subject-store closure, and fresh exact `agent` admission genuinely close; production trust-root/lifecycle proof remains separate; preserve C's block; machine disagreement or agreement still cannot supply German competence or acceptance |
...
- Whole-work lexical observation now exists over the four exact DTA part TEIs:
  ...
  closure were acquired privately. Reviewed `abyss-stack@87d85ad1` now makes
  the materializer invoke the exact `agent` artifact gate before acquisition
  verification, runtime creation, source-packet consumption, or output. The
  real invocation denies the candidate for missing latest eligible promotion,
  failed verification with errors and missing sidecars, and five unverified
  required controls; the exact runtime target is absent. C is
