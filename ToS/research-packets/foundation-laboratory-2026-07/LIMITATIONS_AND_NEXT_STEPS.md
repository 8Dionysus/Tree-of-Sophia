...
The current work establishes a source-addressable corpus spine, local witness
...
The monetary-cost axis now also has a researched, source-free accounting
method at reviewed `abyss-stack` checkpoint `c1758692` and a reviewed mutable
emitter reconciliation at `60c6a9e9`. Reviewed checkpoint `71f8f0b5` adds a
versioned v2 route that verifies exact local evidence bytes, sizes, SHA-256,
reference closure, and one run-receipt row per attempt while explicitly
remaining unsigned and unattested. Reviewed checkpoint `55e58d0a` additionally
requires each exact receipt to satisfy `tos_goal_attempt_run_receipt_v1` and
agree with the ledger's goal, attempt, ordinal, status, UTC interval, terminal
event, and applicable process exit code. Strict JSON parsing rejects duplicate
keys and non-JSON numeric constants. This closes declared-content coherence,
not execution proof. No historical run has fully loaded
monetary cost. The method separates direct billed charges,
whole-system-attributed electricity, hardware allocation, active human time,
and human valuation; it keeps retries and failed attempts inside one goal
boundary. The example's zero external charge is cash-only mechanics, not a
measurement to retrofit onto previous experiments. Without run-specific
energy, an exact tariff, a hardware policy, or human valuation as applicable,
the corresponding monetary fields remain null. The two mutable
Russian-surface emitters now use `unmeasured-local-execution`; digest-bound
translation runners and old receipts remain byte-exact, so their legacy
false-zero wording must be interpreted only through the newer accounting law.
The stronger v2 mechanics do not prove that an execution occurred, authenticate
an evidence producer, capture a complete environment, or prove the truth of a
bill, meter, runtime receipt, result, or human-time statement.
...
by digest in one fail-closed packet. The operator recorded
`human_admitted_with_limits` for citation-only, private, non-commercial
`ai_only` and `ai_human` experiments. That decision admits one critical
citation witness and opens only those two experimental lanes; accepted German,
accepted translation, publisher authentication, German competence,
publication authority, and semantic effects remain zero. The exact frozen A
run was then bound as the first admitted experimental AI-only candidate. Its
obvious Russian-surface defects caused rejection before human review, so no
new human task or correction debt was created.

One later resident Gemma E4B successor capacity probe was executed under the
same admission without reopening the closed historical A/B/C. Its first
episode failed before candidate generation because of runner-contract
incompatibilities and remains negative infrastructure evidence. A bounded
correction was justified only because no model output existed. The corrected
episode returned one schema-valid private candidate, but AI Russian-surface
triage found obvious defects and rejected it before human review. The method,
...
over the invented sentence `Das Fenster ist offen.` returned `Окно открыто.`
and therefore proves only decoding sanity. The owner launch measured
13.377 seconds service time, 59.891 seconds CPU, 3,486.566 MiB peak memory,
...
| German solo+AI assisted route | ordered source-method research, frozen historical five-lane plan, additive participant-scoped exposure plan, one metadata-only eKGWB locator, one reconciled local-only HTTP acquisition, one text-free eKGWB/DTA/Naumann triangulation packet, one FCT/FCCN WARC-backed historical corroboration receipt, one current private-use rights record, one digest-bound citation-witness decision packet, one historical text-free experimental-candidate receipt, three candidate-bearing successor episode receipts, one successor infrastructure-failure receipt, and four exact DTA source-structured part witnesses | prepared units: 30; selected: 1; machine-triangulated: 1; institutional archive corroborations: 1; exact normalized eKGWB↔DTA paragraph/token comparisons: 1; historical admitted candidates: 1; successor infrastructure episodes: 1; successor candidate episodes: 3, of which two rejected and one uncertain; citation decision closed at `human_admitted_with_limits`; routine German backlog: 0; accepted German: 0; admitted critical citation witnesses: 1; open experimental lanes: `ai_only`, exposure-aware `ai_human`; blind human-only lane: unavailable for the current operator; model training exposure: unknown | the decision establishes only edition-typed citation use and honest participant exposure within the declared private limits, not a German-language judgment or independent human baseline. Unauthenticated owner transport, archive non-validation, German-competent acceptance, public sharing, publication, and every promotion route remain open |
...
| translation pre-draft analyses | frozen historical independent-lane schemas plus participant-scoped exposure overlay | 0 packets | machine findings cannot impersonate human evidence; the current-project Antonovsky 2007 page observation forbids a blind-human baseline, while the separate five-year Work-level history remains witness-unspecified |
| translation drafts and adjudication | frozen historical 17-stage plan plus active per-actor exposure, candidate-freeze, comparator-reveal, intervention, and influence-ledger contract | 0 drafts | comparator content stays out of each candidate-producing context until freeze, but historical human memory and unknown model-training exposure remain explicit; no current `human_only` lane exists |
...
## Source and bibliographic limitations
...
- A later exact-identity pass distinguishes the 1898 Bulgakov-journal and
  ...
  *Reader* Edition, and Antonovsky's self-counted 1900 first Edition. The
  ...
  leaves, and the 1898 title page is currently visible only through a
  commercial holding. A following institutional pass resolves one exact 1900
  Edition in the Lithuanian National Library catalog and one physical holding
  under call number `1.40686`; it creates one provisional Edition and one
...
- The Mysl PDF's embedded ABBYY layer contains material corruption and cannot
  be treated as a clean Russian witness.
- Work identity, dating, attribution, edition relations, translations, and
  membership claims beyond the current bounded records remain incomplete and
  may admit competing assertions.
...
- The eKGWB fallback response came from the official public SPA's HTTP include
  ...
  Arquivo.pt snapshot with FCT/FCCN custody, original URL, capture time,
  archive digest, collection, WARC file, and byte offset. Its target block
  matches the 2024-dated owner object and 2026 include, but the archive's terms
  explicitly disclaim source/content validation. The result is independent
  preservation corroboration, not publisher-origin authentication. The
  official-law -> established-work -> fresh-transport refresh also confirms
  that CC BY-NC-ND 4.0 permits private non-commercial adaptation while
  prohibiting its sharing. The payload remains gitignored/local-only; no source
  admission, public derivative, payload publication, or human/legal clearance
  follows from either route.
...
- OCR B rejects one unchanged Party configuration, not Kraken or Party as an
  entire family.
...
- Whole-work lexical observation now exists over the four exact DTA part TEIs:
  ...
  across parts 1, 3, and 4. The first exact ZDL B wheel and its 41-wheel
  dependency closure remain a retained denied attempt. A later import-complete
  42-wheel subject closed all five `local_dev` controls and received the exact
  `agent` allow verdict. Its isolated runtime passed source-free smoke, then
  the three-row packet executed twice with byte-identical streams, three exact
  single-token alignments, and unreviewed `ADV` proposals. The tracked result
  contains only fixity, aggregates, cost, rights, and zero-authority gates;
  source strings and provider lemmas remain private. C is
...
- Neo4j reuses a resident Community service, while Oxigraph owns an isolated
  store and canonical JSON has no database. Their timings are not an equal
  service benchmark.
...
2. freeze exact source revision, artifact digest, license evidence, model
   card, quantization, and runtime configuration;
3. rerun `abyss-machine` storage/resource/thermal admission;
4. acquire only the next admitted candidate rather than the whole option set;
5. record replacement/removal posture for runtime and cache artifacts.

The host thermal owner—not a repo-local invented threshold—governs heavy
work. On this machine 100–105 °C is the monitored active range, values above
...
- The subsequent *Jenseits von Gut und Böse* 1886 provision-identity slice
  ...
  publication, or canon.
- The subsequent 1892 second *Zur Genealogie der Moral* provision-identity
  slice adds two proposed page-region anchors and two exact Edition-owned
  unreviewed claims from different source surfaces. PDF page 5 preserves the
  publisher, Leipzig, and the Edition statement year; PDF page 204 preserves a
  separate, undated printer line. The rebuilt catalog contains 74 objects and
  114 claims; the portable graph contains 386 nodes, 792 claim-centred edges,
  114 source-return traces, and no direct subject-object edge. Exact Edition,
  publisher, printer, and Leipzig queries preserve both roles and both source
...
6. Preserve the completed whole-vocabulary morphology A census and its private
   ...
   froze and executed one output-blind three-context B episode through the
   admitted local-private runtime. Preserve the earlier denied attempt, later
   import-closure fix, private outputs, text-free tracked receipt, and explicit
   zero-authority boundary. Do not schedule a German review merely because the
   provider proposed one repeated label; surface the episode only if a later
   source-grounded decision genuinely needs it. Keep C blocked as
   question-inapplicable until a distinct normalization question exists.
   Machine coverage, alignment, agreement, or disagreement does not create
   accepted German, morphology, a lemma, or `lexeme_id`.
...
8. Keep the E4B successor outside the historical A/B/C namespace. Its first
   no-candidate failure is immutable infrastructure evidence; its corrected
   candidate is immutable private model evidence and rejected before human
...
1. Record an exposure snapshot for every human or model actor. The current
   operator has a five-year Work-level lived route and a separate exact
   Antonovsky 2007 page observation in the current project; a blind-human
   baseline is unavailable. Model-training exposure remains unknown.
2. Produce only an operationally context-isolated AI candidate from cited
   source analysis and a literal interlinear, then freeze its prompt, runtime,
   output, and receipts before comparator reveal.
3. Ask the operator once only after the candidate passes the machine floor and
   a concrete translation question exists. Capture Russian target-language
   response, quick comments or targeted edits, and disclosed remembered
   influence; do not require retyping or per-stage work.
4. Freeze the resulting `exposure-aware-assisted-retranslation` and its
   intervention ledger. Reveal the recognized translation only to the
   designated post-freeze comparison actor and log every post-reveal change.
5. Adjudicate source-facing, target-facing, process-facing, and influence-
   facing evidence separately. German correctness, etymology, fidelity, and
   accepted translation remain blocked without source-language-competent
   human review. Personal read-aloud interpretation stays a separately
   authored layer.
...
1. Execute the ordered discovery protocol and retain exact query/results
   receipts for new witnesses.
2. Ask for human approval before sending any access request.
3. Record permission scope, conditions, expiry, and a redacted public receipt.
4. Review rights and operator approval before changing any server plan from
   metadata-only.
...
| research, decision matrix, machine fit, A/B/C specifications | prepared; morphology A mechanics executed reproducibly; one exact three-context B-only episode was frozen before output and later executed reproducibly through a separately admitted import-complete private runtime; the earlier denied artifact remains negative evidence; C is explicitly question-inapplicable for that episode |
...
| automatic metrics manually rechecked | incomplete; separate Tree recorders independently recompute morphology A aggregates and contextual B source return, token-stream reconstruction, target alignment, repeatability, trust, and resource evidence from private rows, but these remain mechanical verification rather than human content review |
| quality, cost, speed, and human correction time | morphology A, contextual B, historical translation A/B/C, E4B, and Qwen3 8B CPU have bounded speed/resource evidence. Contextual B now separates the retained 356.746-second negative acquisition, corrected source-free runtime build, and exact private execution: provider pass 1 took 0.098123434 s, owner execution 11.118 s, process peak RSS was 1,273,647,104 bytes, and owner footprint/swap peaks were 1,892.715 MiB/0.25 MiB; accuracy and correction time remain unmeasured. The specialized MADLAD route adds a 1,671,227,060-byte three-file model set, a 9,667,291-byte runtime, a 264.620562-second source-free caller episode, and one 979.291729-second private call dominated by a 901.152729-second owner gate; its worker took 71.241677 seconds and peaked at 3,330,088,960 bytes RAM plus 667,648 bytes swap; the deterministic Russian floor measured about 7.707 ms/candidate in-process and about 289.050 ms/candidate with required resident Hunspell on invented controls; pinned offline LanguageTool B measured 78.004763012 s wall and 600,440 KiB peak child RSS over 36 isolated cold starts; Qwen3.5 GEC C measured 111.711071750 s wall and a 3.5 GiB cgroup peak over nine batches, with 3/15 exact corrections, two clean false changes, and zero process swap; one real-human OCR candidate-review pass recorded 8,334 seconds, but display position, page difficulty, and the single pass confound method comparison; reviewed checkpoint `7f28312e` proves the current energy-observation route is `blocked-counter-permission` without rerunning a model; reviewed checkpoints `c1758692`, `60c6a9e9`, `71f8f0b5`, and `55e58d0a` prove cash/marginal/fully-loaded/per-successful-goal accounting mechanics, reconcile mutable false-zero emitters, verify byte-exact v2 evidence closure, require one run receipt per attempt, and enforce receipt/ledger/time/terminal coherence without assigning costs to old runs, proving execution, authenticating evidence producers, or rewriting fixed runner bytes; fully loaded monetary cost, run energy, stable correction cost, and accepted content quality remain unmeasured, while the earlier Qwen3 8B translation model-process peak was not captured by its owner wrapper |
| A/B/C reproducibility | demonstrated for bounded mechanical slices only; morphology A has two byte-identical whole-vocabulary passes, contextual B has a reproducibly materialized input and two byte-identical three-row provider streams, and C is question-inapplicable; Structure B/C have one corrected full-scope run each under different frozen scopes |
...
The foundation should not be labeled complete until the human rows above are
closed with real receipts. The appropriate handoff is not “rerun the
validator”; it is “perform the next source-visible human decision, then return
through the same evidence chain.”
