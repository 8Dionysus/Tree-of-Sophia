...
| C01 · G6 | общий язык разрешённого роста для человека и агента | Growth → create, revise, assert, assess, merge/split proposal, extend | понятная команда, diff и конфликт | create/revise форм, historical/declared metadata и отдельных Claim; [явная составная Work → Expression команда и адресная ревизия native-метаданных](../review-ledger/2026-09-09-native-work-expression-growth-review.md) сохраняют предшественников, проверяют полномочия, повтор и восстановление без обхода потомков. Старые flat-package grants не расширены; остальные предметные команды ещё нужны; **в работе** |
