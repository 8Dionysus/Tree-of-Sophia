...
Матрица сведена на source baseline
...
До **закрытия** v1 остаются четыре сквозных gate, а не новый бесконечный список
...
- **K4 — отображение источников и остаток:** source-enumerated public universe,
  ...
  продолжения. Число узлов не определяет знаменатель и не доказывает качество.
...
| F04 · M15/M19, G2 | тождество, сходство, отношение и преемство | Registry/Growth → compare, propose merge/split, resolve old ID | основания объединения и прежние чтения | **локальное исполнение предложений только source identities**: [merge/split proposal](../review-ledger/2026-09-09-identity-transition-proposals-review.md) связывает полные exact predecessor/successor sets, basis и migration plan, типы — из owner descriptor. Ложные совпадения и неполные sets отклоняются; старые ID/история сохраняются. Это не выполненный destructive merge, alias или самовыданный допуск. Concept/Conception/Lexeme и другие semantic-object subjects пока исключены `eligible_type`; это действующий пробел F04/C01, а не требование destructive merge. Владелец Growth должен добавить exact typed semantic-subject route с отдельной проверкой полномочий, версий, сохранности и отказов до K1/K2. |
...
| M01 · G8 | прямое отображение, адаптер, уточнение, неизвестность, ограничение | Corpus/Processing → shadow migration, classify remainder | статус каждого объекта и следующий владелец | **source return реализован; K4/K1 открыты**: [catalog audit](../review-ledger/2026-09-09-source-projection-coverage-review.md), [пять legacy Link Claims](../review-ledger/2026-09-09-legacy-object-link-context-review.md), [253 CSV/92 nodes](../review-ledger/2026-09-09-corpus-csv-retention-review.md), [16137 atlas rows/4 manifests](../review-ledger/2026-09-09-philosophy-atlas-source-return-review.md) и [17599 backlog occurrences](../review-ledger/2026-09-09-philosophy-backlog-source-return-review.md) сохраняют fields/IDs/refs/status. Исторические 665 не final denominator. Source-enumerated audit отдельно проверяет public и ограниченный private metadata universe, остаток и derived currentness без disclosure/автопринятия. |
