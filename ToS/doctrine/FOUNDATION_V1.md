...
| L02 · M15/M19/M25 | текстовый слой, адрес, фрагмент, единица, употребление, форма, лексема и значение | Corpus/Registry → focus word/occurrence, source return | слово в точном контексте | **частично, реальный лексический срез связан**: Lexeme, LexicalForm и LexicalSense имеют содержательные профили, общий рост, формы и читатель; Wille/Willen и ограниченное чтение JGB 19 связаны отдельными квалифицированными Claims ([проверка](../review-ledger/2026-09-08-lexical-profile-review.md)). Строка формы сохраняется без тихой нормализации, Sense не Concept, закрытые native ID не переиспользуются. Добавлены точный native TextUnit/Anchor return и v3 owner-local assessment adapter с реальным DTA describe/inspect без допуска ([проверка](../review-ledger/2026-09-08-native-text-return-review.md)). Occurrence, полный текстовый рост, содержательная оценка и UI ещё не завершены |
