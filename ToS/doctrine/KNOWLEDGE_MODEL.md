...
while its authored home has moved into `ToS/canon/synthesis/`.
...
- `ToS/public-compatibility/concept_node.example.json` is the current public compatibility bounded hop
- `ToS/doctrine/KNOWLEDGE_MODEL.md` remains the in-repo fallback orientation surface
...
- `ToS/source-witnesses/` for primary witness and source files
- `ToS/candidate-intake/` for raw extracted candidate material and current candidate tabular base packs
- `ToS/canon/` for canonical authored nodes, relations, and vocabulary governance surfaces
...
- `ToS/candidate-intake/.../mode-b/*.csv` remains the candidate tabular base pack
- `ToS/canon/event/.../node.json` and `ToS/canon/state/.../node.json` now carry the
  reviewed dynamic canon promoted from that pack
- `ToS/canon/support/.../node.json` now carries the reviewed full non-literal
  support canon from the bounded `n.*` layer
- `ToS/canon/analogy/.../node.json` now carries the bounded bee-honey analogy as a
  reviewed image-bearing canon surface
- `ToS/canon/synthesis/.../node.json` now carries the route-local
  departure-from-reflective-origin reading as reviewed synthesis rather than
  deferred commentary residue
- `ToS/canon/relations/.../edges.csv` now carries the reviewed canonical relation
