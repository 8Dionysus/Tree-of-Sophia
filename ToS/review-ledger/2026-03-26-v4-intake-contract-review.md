...
- `yes` `ToS/source-witnesses/`, `ToS/candidate-intake/`, and `ToS/canon/` remain visibly distinct
...
- `yes` raw tables stay candidate-only and do not promote into `ToS/canon/`
...
- bridge rows remain included as candidate reasoning structure, but they are not
  yet promoted to any canonical `ToS/canon/` relation layer
- future promotion into `ToS/canon/` should decide what survives as stable node law
...
from `ToS/candidate-intake/` into canonical `ToS/canon/`.
