# 2026-03-22 Pre-Expansion Soil Review

## What changed

- added a public context-node template
...
- `yes` context law was added without opening a new corpus slice
- `yes` tree motion remains gated by maintainer-led manual source entry
- `yes` wider world-thought expansion remains later rather than being half-opened here
- `yes` ToS ownership remains primary and AoA support remains downstream
...
Follow-up belongs first in `Tree-of-Sophia` until manual source entry begins.
Only after that should neighboring support repos restage the next growth move.
