...
- updated `ToS/doctrine/TINY_ENTRY_ROUTE.md` so it records the current downstream consumption posture rather than describing it as a later possibility
- updated `ToS/doctrine/KNOWLEDGE_MODEL.md` so the tiny-entry seam is described as already consumable downstream without weakening ToS authority
