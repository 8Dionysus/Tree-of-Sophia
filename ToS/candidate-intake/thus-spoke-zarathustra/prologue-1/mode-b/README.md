...
Primary witness material remains in `ToS/source-witnesses/`.
Canonical authored source-node law remains in `ToS/canon/`.
...
from `ToS/candidate-intake/` into `ToS/canon/`.
...
- `ev.p5.bee_honey_analogy` remains in `ToS/candidate-intake/` as `deferred_analogy`

For the current support ledger, that now means:

- all 46 non-literal `n.*` rows in `nodes.csv` are marked `promoted`
- `literal.ten_years` and `literal.too_much` remain in `ToS/candidate-intake/` as
  `deferred_literal`

For the current edge ledger, that now means:

- 122 `edges.csv` rows are marked `promoted`
- 3 rows remain in `ToS/candidate-intake/` as `deferred_literal`
- 2 rows remain in `ToS/candidate-intake/` as `deferred_analogy`
- 1 row remains in `ToS/candidate-intake/` as `deferred_commentary`
