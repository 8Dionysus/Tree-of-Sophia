...
test('existing preferences gain control defaults without losing docking, tools or panel sizes',()=>{
  ...
  expect(readInterface(storage)).toEqual({...old,inputMode:'trackpad',motion:'system'});
...
test('persisted control choices roundtrip and reject unsupported values before application',()=>{
  const preferences={...DEFAULT_INTERFACE,inputMode:'mouse',motion:'paused',sizes:{settings:{width:430,height:560}}};
  expect(validateInterface(JSON.parse(JSON.stringify(preferences)))).toEqual(preferences);
  for(const delta of [{motion:'sometimes'},{inputMode:'tablet'},{v:2},{pinned:['settings']},{sizes:{settings:{width:99999,height:560}}}])
