...
export function nodePreview(packet,node,preferred=uiLanguage()){
  ...
  const connections=(packet?.relations||[]).filter(r=>r.from_id===node?.id||r.to_id===node?.id).map(relation=>{
    ...
    const inverse=localized(relation.display?.inverse_label,'',preferred),label=localized(relation.display?.label,'',preferred),name=displayTitle(other,'',preferred);
    if(!name)return '';
    return outgoing?label+' → '+name:inverse?inverse+': '+name:name+' → '+label;
  }).filter(Boolean);
  return {kind:compact(kind,72),title:compact(displayTitle(node,'',preferred),100),body:compact(summary||[...new Set(connections)].slice(0,2).join(' · '),140)};
}
