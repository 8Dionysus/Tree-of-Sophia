...
export function mountScene(root, {client,onChange=()=>{}, initialFocus,initialLens,autoStart=true}={}) {
  ...
  function ensureOverlay(kind){const el=q('.sc-'+kind);if(!el.hidden)return;const restore=!panel.hidden||overlayReturnPanel;closeSearch(false,false);closeLenses(false,false);overlayReturnPanel=restore;panel.hidden=true;el.hidden=false;uiAttribute(q('.sc-'+kind+'-open'), 'aria-expanded', 'true');if(kind==='search'){search();q('#sc-query').focus()}else q('.sc-lens[aria-pressed="true"]').focus();layoutDirty=true;kick();}
  function openOverlay(kind){const el=q('.sc-'+kind);if(!el.hidden){kind==='search'?closeSearch():closeLenses();return}ensureOverlay(kind);}
  ...
  refreshIcons();
  return {port:scenePort, ui:knowledgeUI, openSearch:()=>openOverlay('search'), ensureSearch:()=>ensureOverlay('search'), overview, closeInspector, invalidate:()=>{layoutDirty=true;kick()}};
}
