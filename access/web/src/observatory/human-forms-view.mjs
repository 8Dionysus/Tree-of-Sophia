...
import {formView} from './human-forms.mjs';
...
export function renderHumanForms(raw){
  ...
  for(const selected of view.roles){
    ...
    if(selected.state!=='ready'){
      ...
      wording.dataset.readingAnchor='form:'+selected.role+':wording';section.append(wording);
      if(packet.context.length){
        ...
        packet.context.forEach((entry,index)=>{
          ...
          context.append(jsonBlock(entry.value,'form:'+selected.role+':context:'+index));
          // Bindings and unknown context members are retained alongside value.
          const {value,...binding}=entry;context.append(jsonBlock(binding,'form:'+selected.role+':binding:'+index));section.append(context);
        });
      }
      if(packet.language_context){section.append(el('h6',ui('Языковое происхождение')),jsonBlock(packet.language_context,'form:'+selected.role+':language'));}
      const metadata=el('details','','sc-form-metadata');metadata.append(el('summary',ui('Происхождение и точная версия формы')));
      const {display_text,context,language_context,...rest}=packet;metadata.append(jsonBlock(rest,'form:'+selected.role+':metadata'));section.append(metadata);
