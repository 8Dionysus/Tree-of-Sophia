...
export const DEFAULT_INTERFACE={v:1,pinned:['search','lenses','workspace','navigation'],dock:'auto',text:'comfortable',labels:'normal',inputMode:'trackpad',motion:'system',sizes:{}};
const panelIds=['inspector','workspace','evidence','navigation','builder','studio','reader','history','copy','settings'];
export function validateInterface(value){
  ...
  const inputMode=value.inputMode??'trackpad',motion=value.motion??'system';
  if(!['trackpad','mouse'].includes(inputMode)||!['system','paused','running'].includes(motion))throw new Error('Настройки управления повреждены.');
  ...
  return {v:1,pinned:[...value.pinned],dock:value.dock,text:value.text,labels:value.labels,inputMode,motion,sizes};
}
