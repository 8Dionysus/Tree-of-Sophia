...
export function validatePose(value){
  if(!value||!['constellations','plane','orbits'].includes(value.lens)||!bounded(value.yaw,-100000,100000)||!bounded(value.pitch,-2,2)||!bounded(value.zoom,.1,5)
    ...
    ||typeof value.panelOpen!=='boolean'||!['about','relations'].includes(value.cardTab)||!Array.isArray(value.vertices)||value.vertices.length>40)throw new Error(t("Не удалось прочитать положение сохранённого места."));
  const ids=new Set();
  const vertices=value.vertices.map(v=>{
    if(!id(v.id)||!v.id||ids.has(v.id)||!Number.isInteger(v.slot)||v.slot<0||v.slot>1000||!vector(v.p)||!vector(v.sourcePosition)||!vector(v.target)||!bounded(v.volumeZ,-10000,10000))throw new Error(t("Сохранённое расположение звёзд повреждено."));
