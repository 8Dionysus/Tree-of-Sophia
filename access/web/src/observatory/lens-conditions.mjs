...
export const operatorLabels={eq:'равно',neq:'не равно',in:'одно из',contains:'содержит',prefix:'начинается с',exists:'значение указано',gt:'больше',gte:'не меньше',lt:'меньше',lte:'не больше'};
...
export function validateConditions(value){
  if(!value||typeof value!=='object'||Array.isArray(value)||Object.keys(value).some(k=>!['nodes','relations'].includes(k)))fail('Неверный набор условий.');
  const result={};
  for(const kind of ['nodes','relations']){
    const entries=value[kind];
    if(!Array.isArray(entries)||entries.length>MAX_CONDITIONS)fail('Можно добавить до 12 условий в каждый раздел.');
    result[kind]=entries.map(rule=>{
      if(!rule||!['field','property_id'].includes(rule.selector)||!id(rule.id)||!Object.hasOwn(operatorLabels,rule.op)
        ...
        ||kind==='relations'&&rule.selector==='property_id')fail('Условие неполно или имеет неподдерживаемое значение.');
...
export function conditionCatalog({catalog,schema},kind){
  ...
  for(const field of caps[wireKind+'_fields']||[]){
    ...
    if(ops.length)entries.push({selector:'field',id:field,title,valueType,operators:ops,
...
export function compileConditions(rules,context,kind){
  ...
  if(rules.length&&(!Number.isInteger(limit)||rules.length+1>limit))fail('Сервер допускает меньше условий в этом разделе.');
  return rules.map(rule=>{
    const entry=entries.find(item=>conditionKey(item)===conditionKey(rule));
    if(!entry||!entry.operators.includes(rule.op))fail('Условие больше не поддерживается: '+(entry?.title||rule.id)+'. Измените или удалите его.');
    const values=Array.isArray(rule.value)?rule.value:[rule.value],type=entry.valueType==='string-array'?'string':entry.valueType;
    if(rule.op==='exists'){
      if(typeof rule.value!=='boolean')fail('Для наличия значения выберите «да» или «нет».');
    }else if(!values.length||values.some(value=>typeof value!==type||typeof value==='number'&&!Number.isFinite(value)))fail('Проверьте тип значения: '+entry.title+'.');
    if(['eq','neq','prefix','gt','gte','lt','lte'].includes(rule.op)&&Array.isArray(rule.value))fail('Эта операция принимает одно значение.');
    if(rule.op==='contains'&&Array.isArray(rule.value)&&entry.valueType!=='string-array')fail('Для поиска внутри текста нужно одно значение.');
    return {[rule.selector]:rule.id,op:rule.op,value:structuredClone(rule.value)};
  });
}
export function conditionText(rule,entries){
  const entry=entries.find(item=>conditionKey(item)===conditionKey(rule));
  const value=rule.op==='exists'?(rule.value?'да':'нет'):Array.isArray(rule.value)?rule.value.map(v=>JSON.stringify(v)).join(', '):JSON.stringify(rule.value);
