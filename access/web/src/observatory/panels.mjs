...
export function createPanelHost(root,scene,{onUserAction=()=>{}}={}){
  ...
  const title=id=>id==='inspector'?'К карточке':({evidence:'К основаниям',workspace:'К источникам',navigation:'К маршруту',builder:'К линзе',studio:'К рабочему месту'})[id]||'Назад';
  ...
  function back(){
    onUserAction();const stamp=key();let previous;while(trail.length){const candidate=trail.pop();if(candidate.key===stamp){previous=candidate;break;}}
  ...
  function updateBack(){for(const entry of panels.values()){const last=[...trail].reverse().find(item=>item.key===key());entry.back.hidden=!last;entry.back.textContent=last?'← '+title(last.id):'';}}
  function show(id,record=true){
    ...
    if(id!=='inspector'){const context=root.querySelector('.sc-context').getBoundingClientRect();element.style.setProperty('--sc-tool-top',`${context.bottom-root.getBoundingClientRect().top+18}px`);element.dataset.dock=dock;}
  ...
  function register(id,element,onHide=()=>{},options={}){
    ...
    const size=document.createElement('button');size.type='button';size.className='sc-panel-size';size.textContent='↔';size.setAttribute('aria-label','Изменить размер окна');size.title='Размер окна: обычный / просторный';
    ...
    const grip=document.createElement('button');grip.type='button';grip.className='sc-panel-resize';grip.textContent='⌟';grip.setAttribute('aria-label','Размер окна: стрелки меняют ширину и высоту');grip.title='Потяните угол; стрелки меняют размер, Home сбрасывает';
    let drag=null;
  ...
  observer.observe(root,{attributes:true,attributeFilter:['data-graph-revision','data-selected']});
  ...
  return {register,open:show,close,back,configure(id,options){Object.assign(panels.get(id),options);},
    ...
    addTool(id,tool){if(tools.has(id))throw new Error('Duplicate tool: '+id);tools.set(id,tool);applyPreferences();},
