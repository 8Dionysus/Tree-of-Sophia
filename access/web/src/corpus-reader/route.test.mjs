...
test('scope keeps source rows, folded display and unknown corpus count distinct',()=>{
  ...
  assert.equal(scope.corpusTotal,undefined);assert.match(scopeText(scope,'en'),/corpus total is not supplied/);
  state.view.continuation.next_cursor=null;assert.equal(explorationScope(state).hasMore,false);
});
