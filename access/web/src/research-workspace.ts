...
const MAX_ID = 256; const MAX_LABEL = 512; const MAX_TEXT = 4_000; const MAX_ITEMS = 256;
...
export type ResearchWorkspaceState = {
  ...
  hypotheses: ResearchHypothesis[]; notes: WorkspaceNote[]; journal: WorkspaceJournalEntry[];
};
...
export type ResearchWorkspaceSummary = { schema: "tos_research_workspace_summary_v1"; session_id: string; revision: number; hypothesis_count: number; excluded_edge_count: number; comparison_count: number; note_count: number; journal_count: number; can_undo: boolean; can_redo: boolean };
...
function cloneState(state: ResearchWorkspaceState): ResearchWorkspaceState {
  return { ...state, selectedLens: state.selectedLens ? { ...state.selectedLens } : null, excludedEdgeIds: [...state.excludedEdgeIds],
    ...
    hypotheses: state.hypotheses.map((item) => ({ ...item, posture: { ...item.posture } })), notes: state.notes.map((item) => ({ ...item })),
...
function stateOf(packet: unknown): ResearchWorkspaceState {
  ...
  if (!isRecord(packet) || !only(packet, required) || packet.schema !== RESEARCH_WORKSPACE_SCHEMA || packet.version !== RESEARCH_WORKSPACE_VERSION) throw new Error("packet shape or schema is invalid");
  ...
  if (routes.length > MAX_ITEMS || hypotheses.length > MAX_ITEMS || notes.length > MAX_ITEMS) throw new Error(`collections exceed ${MAX_ITEMS} items`);
  if (new Set(routes.map((item) => item.id)).size !== routes.length || new Set(hypotheses.map((item) => item.id)).size !== hypotheses.length || new Set(notes.map((item) => item.id)).size !== notes.length) throw new Error("collection IDs must be unique");
  return { schema: RESEARCH_WORKSPACE_SCHEMA, version: RESEARCH_WORKSPACE_VERSION, sessionId: idValue(packet.session_id, "session_id"), revision: integerValue(packet.revision, "revision", MAX_REVISION), selectedLens: selectionOf(packet.selected_lens), excludedEdgeIds: listValue(packet.excluded_edge_ids, "excluded_edge_ids", MAX_ITEMS), routeSnapshots: routes, hypotheses, notes, journal: journalOf(packet.journal) };
}
...
function emptyState(sessionId: string): ResearchWorkspaceState { return { schema: RESEARCH_WORKSPACE_SCHEMA, version: RESEARCH_WORKSPACE_VERSION, sessionId, revision: 0, selectedLens: null, excludedEdgeIds: [], routeSnapshots: [], hypotheses: [], notes: [], journal: [] }; }
...
export function createResearchWorkspace(options: ResearchWorkspaceOptions = {}) {
  ...
  const journal = (draft: ResearchWorkspaceState, action: string, targetId?: string): void => { const sequence = (draft.journal.at(-1)?.sequence ?? 0) + 1; draft.journal.push({ sequence, action, ...(targetId ? { targetId } : {}) }); if (draft.journal.length > MAX_JOURNAL) draft.journal.splice(0, draft.journal.length - MAX_JOURNAL); };
  ...
  return {
    ...
    summary: (): ResearchWorkspaceSummary => ({ schema: "tos_research_workspace_summary_v1", session_id: state.sessionId, revision: state.revision, hypothesis_count: state.hypotheses.length, excluded_edge_count: state.excludedEdgeIds.length, comparison_count: state.routeSnapshots.length, note_count: state.notes.length, journal_count: state.journal.length, can_undo: undoStack.length > 0, can_redo: redoStack.length > 0 }),
    ...
    removeHypothesis: (id: string): ResearchWorkspaceState => { const targetId = idValue(id, "hypothesis.id"); return !state.hypotheses.some((item) => item.id === targetId) ? cloneState(state) : commit("hypothesis.remove", (draft) => { draft.hypotheses = draft.hypotheses.filter((item) => item.id !== targetId); }, targetId); },
