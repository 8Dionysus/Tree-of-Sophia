...
class ToSAccessCore:
    ...
    def source_descend(
        ...
        if self._prepared_reader is not None:
            return self._prepared_reader.source_descend(node_id, max_depth=bounded_depth, limit=bounded_limit)
    ...
    def source_dossier(self, object_id: str, limit: int = 300) -> dict[str, Any]:
        ...
        bounded_limit = _bounded_int(limit, 300, 1, 300)
        if self._prepared_reader is not None:
            return self._prepared_reader.source_dossier(object_id, limit=bounded_limit)
        store = self._query_store()
        navigation = store.metadata['source_navigation_header'] if store else self.source_navigation(bibliographic_only=True)
        ...
        rights_for = store.rights if store else lambda component_ids: navigation.get("rights", [])
