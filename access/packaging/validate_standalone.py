    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="tos-standalone-validate-") as raw_temp:
        ...
        env = {
            ...
            if key not in {"TOS_DATA_ROOT", "TOS_ROOT", "AOA_TOS_ROOT", "TOS_QUERY_STORE_PATH", "PYTHONPATH"}
        }
...
def main() -> None:
    ...
    mode.add_argument("--software", action="store_true", help="validate software contracts without loading a data snapshot")
    ...
    result = (
        ...
        if args.bundle
        else (validate_software(args.repo_root.resolve()) if args.software else validate_source(args.repo_root.resolve()))
    )
