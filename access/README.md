...
Native navigation now requires per-row `source_navigation_row_digest` metadata
for nodes, edges and rights, plus `source_navigation_header_digest` for the
authority-bearing header. Python and D1 verify exact emitted JSON (including
hydrated chunks) before deriving a dossier. Full, initial-product and addressed
delta producers maintain these companions. Missing checksums require explicit
product migration; they are never generated by a reader from untrusted serving
bytes. The existing native bootstrap module also exposes
`build_source_navigation_integrity_sql`: it compares the complete native product
with independently admitted immutable navigation/rights inputs and emits guarded
forward/reverse checksum-only SQL. Its explicit `header_only` continuation
compares the header with admitted immutable inputs without repeating or claiming
a row audit; it leaves existing row companions intact. It does not rewrite corpus rows or grant
rights. Reversing that migration requires a compatible older consumer; the
new reader refuses the checksum-less product. These checks detect persisted
carrier drift, not a malicious publisher able to replace both rows and their
checksums. Source, rights and publication admission remain independent.
