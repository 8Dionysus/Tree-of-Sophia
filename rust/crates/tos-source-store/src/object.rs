...
pub(crate) fn verify_selected_object(
    ...
    if expected_size > max_bytes {
        return Err(StoreError::new(StoreErrorCode::BudgetExceeded, "selected object exceeds read limit"));
    }
    ...
    if !directory.is_dir() || directory.file_type().is_symlink() {
        return Err(StoreError::new(StoreErrorCode::UnsafePath, "object directory is not a regular directory"));
    }
    let path = object_dir.join(sha256.to_hex());
    let metadata = fs::symlink_metadata(&path)
        .map_err(|error| StoreError::io("cannot stat selected object", error))?;
    if !metadata.is_file() || metadata.file_type().is_symlink() {
        return Err(StoreError::new(StoreErrorCode::UnsafePath, "selected object is not a regular file"));
    }
    if metadata.len() != expected_size {
        return Err(StoreError::new(StoreErrorCode::CorruptSelectedObject, "selected object size differs"));
    }
    let mut file = File::open(&path)
        .map_err(|error| StoreError::io("cannot open selected object", error))?;
    ...
    loop {
        ...
        let count = file.read(&mut block[..request])
        ...
        if actual > max_bytes || actual > expected_size {
            return Err(StoreError::new(StoreErrorCode::CorruptSelectedObject, "selected object grew during read"));
        }
    ...
    if actual != expected_size || digest.finalize() != sha256 {
        return Err(StoreError::new(StoreErrorCode::CorruptSelectedObject, "selected object digest or size differs"));
    }
    Ok(actual)
}

pub(crate) fn verify_without_copy(root: &Path, sha256: Digest256, expected_size: u64, max_bytes: u64) -> Result<()> {
