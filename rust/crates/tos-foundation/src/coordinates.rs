    ...
    pub fn new(start: u64, end: u64) -> Result<Self> {
        if start > end {
            return Err(FoundationError::new(
                FoundationErrorCode::InvalidCoordinate,
                "end precedes start",
            ));
        }
        Ok(Self { start, end })
...
impl CodePointSpan {
    pub fn new(
        ...
        representation_digest: Digest256,
        normalization_id: &str,
    ) -> Result<Self> {
        if start > end
            || normalization_id.is_empty()
            || normalization_id.chars().any(char::is_control)
        {
            return Err(FoundationError::new(
                FoundationErrorCode::InvalidCoordinate,
                "invalid code-point span or normalization id",
            ));
        }
        Ok(Self {
            start,
    ...
    pub fn byte_span_in(&self, representation: &str) -> Result<ByteSpan> {
        ...
        if self.end > total {
            return Err(FoundationError::new(
                FoundationErrorCode::InvalidCoordinate,
                "code-point interval exceeds representation",
            ));
        }
        let byte_at = |index: u64| -> u64 {
            if index == total {
                representation.len() as u64
            } else {
                representation
                    .char_indices()
                    .nth(index as usize)
                    .map_or(0, |(byte, _)| byte as u64)
            }
        };
