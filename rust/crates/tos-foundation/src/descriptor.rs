...
impl ContractDescriptor {
    pub fn new(
        ...
        Ok(Self {
            ...
            schema_digest,
            codec_profile: codec_profile.to_owned(),
        })
    }
}
...
impl OperationDescriptor {
    pub fn new(
        operation_id: &str,
        version: &str,
        input_contract: ContractKey,
        output_contract: ContractKey,
...
impl DescriptorRegistry {
    ...
    pub fn register_operation(&mut self, descriptor: OperationDescriptor) -> Result<()> {
        if !self.contracts.contains_key(&descriptor.input_contract)
            || !self.contracts.contains_key(&descriptor.output_contract)
        {
            return Err(FoundationError::new(
    ...
    pub fn contract(&self, key: &ContractKey) -> Option<&ContractDescriptor> {
    ...
    pub fn contracts(&self) -> impl Iterator<Item = &ContractDescriptor> {
...
fn validate_name(value: &str) -> Result<()> {
    if value.is_empty() || value.chars().any(char::is_control) {
        return Err(FoundationError::new(
            Code::InvalidDescriptor,
            "descriptor field is empty or contains a control character",
        ));
    }
    Ok(())
}
