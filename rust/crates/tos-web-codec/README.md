...
and an explicit profile. It returns a structured result with exact output
...
installed-artifact checks. The independent vectors live in
`tests/conformance/rust/foundation.jsonl`.
...
pass the generated JS, generated WASM and fixture paths to
`tests/wasm-host.mjs`. Node's WebAssembly runtime executes the same Rust
