...
Wrangler adapter uses the current ``r2 object put/get`` commands.  The
optional REST adapter reuses Wrangler's supported in-memory auth command and
keeps the same upload/readback/receipt contract.  Both single-object paths
are intentionally bounded at 300 MiB; a multipart adapter is future work and
is not silently substituted here.
"""
...
def _reviewed_source_postures(context: PlanContext, rights: dict[str, Any]) -> tuple[set[str], set[str], set[str]]:
    """Resolve reviewed historical admission defaults, never license denials.

    The exact source record stays unchanged. An explicit source-bound review
    may explain why its old admission defaults do not forbid licensed private
    byte retention. No inferred or provider-wide permission is created here.
    """
    policy = context.plan["rights_policy"]
    if policy["review_status"] != "agent-reviewed":
        return set(), set(), set()
    review = load_json(safe_repo_path(context.repo_root, policy["rights_review"]["ref"]))
    resolution = review.get("source_posture_resolution")
    if resolution is None:
        return set(), set(), set()
    manifest_path, _manifest, rights_path, _rights = _manifest_and_rights(context)
    _validate_agent_review(context, manifest_path=manifest_path, rights_path=rights_path)
    if any(v not in {"licensed", "open_licensed", "public_domain_reviewed", "public_domain", "permission_granted"}
           for v in _rights_posture_values(rights, "assessment_status")):
        raise RightsGateError("source posture resolution requires positive assessments for every layer")
    if any(entry["state"] != "prohibited" for entry in context.plan["allowed_derivatives"].values()):
        raise RightsGateError("source posture resolution only permits exact-byte private retention")
    server = set(resolution["server_processing_postures"])
    redistribution = set(resolution["redistribution_postures"])
    expected_server = set(_rights_posture_values(rights, "server_processing_posture")) & {"not_authorized", "authorized_with_conditions"}
    expected_redistribution = set(_rights_posture_values(rights, "redistribution_posture")) & {"not_authorized", "metadata_only", "authorized_with_conditions"}
    layers = rights.get("layer_assessments", [])
    if not isinstance(layers, list):
        layers = []
    source_conditions = {
        condition for container in [rights, *layers] if isinstance(container, dict)
        for condition in container.get("restrictions", []) if isinstance(condition, str)
    }
    retained = resolution.get("retained_conditions", [])
    retained_conditions = {entry["source_condition"] for entry in retained}
    if retained_conditions != source_conditions or len(retained_conditions) != len(retained):
        raise RightsGateError("source condition review must retain every exact condition once")
    if server != expected_server or redistribution != expected_redistribution or not (server or redistribution or retained_conditions):
        raise RightsGateError("source posture resolution does not exactly match historical admission defaults")
    return server, redistribution, retained_conditions
...
def _assert_rights_record_compatible(context: PlanContext, rights: dict[str, Any]) -> None:
    ...
    reviewed_server, reviewed_redistribution, retained_conditions = _reviewed_source_postures(context, rights)
    if set(restrictions) - retained_conditions:
    ...
    unsupported_server = set(server_postures) - KNOWN_SERVER_POSTURES - BLOCKED_RIGHTS_STATUSES - reviewed_server
    if unsupported_server:
        raise RightsGateError("rights record has an unsupported server-processing posture")
    if any(value in BLOCKED_RIGHTS_STATUSES - reviewed_server for value in server_postures):
    ...
    unsupported_redistribution = set(redistribution) - KNOWN_REDISTRIBUTION_POSTURES - BLOCKED_RIGHTS_STATUSES - reviewed_redistribution
    if unsupported_redistribution:
        raise RightsGateError("rights record has an unsupported redistribution posture")
    if any(value in BLOCKED_RIGHTS_STATUSES - reviewed_redistribution for value in redistribution):
    ...
    for verified in verified_files:
        with _import_lock(receipt_dir):
            with tempfile.TemporaryDirectory(prefix="tos-r2-readback-", dir=str(scratch_root)) as temp_dir:
                ...
                # operator-local critical section. The selected OAuth REST or
                # Wrangler transport has no conditional-create primitive, so
                # this does not claim a cross-host compare-and-swap.
...
def _build_transport(args: argparse.Namespace, *, max_upload_bytes: int | None = None) -> TransportAdapter:
    """Build the selected adapter without changing the custody protocol."""

    if args.transport == "rest":
        if not args.account_id:
            raise TransportError("--account-id is required with --transport rest")
        try:
        ...
        except R2TransportError as exc:
            raise TransportError(str(exc)) from exc
    return WranglerR2Transport(
        bucket=args.bucket,
        executable=args.wrangler,
        max_upload_bytes=(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    transfer.add_argument("--transport", choices=("wrangler", "rest"), default="wrangler")
    transfer.add_argument("--account-id", help="required for the Cloudflare REST transport")
    ...
    read_remote_parser.add_argument("--transport", choices=("wrangler", "rest"), default="wrangler")
    read_remote_parser.add_argument("--account-id", help="required for the Cloudflare REST transport")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "import":
            ...
            context = load_plan(args.repo_root, args.plan)
            transport = _build_transport(args, max_upload_bytes=args.max_upload_bytes)
        ...
        if args.command == "registry-disable":
            ...
            print(json.dumps(entry, ensure_ascii=False, indent=2))
            return 0
    except (SourcePayloadImportError, R2TransportError) as exc:
