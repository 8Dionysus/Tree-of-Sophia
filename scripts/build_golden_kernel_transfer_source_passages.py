...
ANTI_UNRESOLVED = {
    "main:8": ("start", "end_exclusive"),
    "main:9": ("start",),
    "main:43": ("end_exclusive",),
    "main:44": ("start",),
}
...
def _numeric_candidate(value: str) -> str:
    ...
    return compact.translate(str.maketrans({"i": "1", "l": "1", "x": "1", "o": "0", "s": "5", "z": "2"}))
...
def _manifest_entry(
    *, repo_root: Path, payload_root: Path, item_dir: Path, media_type: str
    ...
    payload_path = (
        payload_root
        / item_dir.relative_to(SOURCE_ROOT)
        / entry["relative_path"]
    )
    if (
...
def _pdf_pages(
    ...
    for page_number, page in enumerate(elements, start=start_page):
        ...
        records.sort(key=lambda record: (record["y_min"], record["x_min"], record["order"]))
        pages[page_number] = {
            "page": page_number,
...
def _djvu_pages(path: Path, *, expected_count: int, body_y_min: float) -> dict[int, dict[str, Any]]:
    try:
...
def _pdf_marker(pages: dict[int, dict[str, Any]], page: int, key: str) -> dict[str, Any]:
    matches = [
        record
        for record in pages[page]["records"]
        if 150 <= record["x_min"] <= 220
    ...
    return {**matches[0], "marker_basis": "exact-normalized-label-in-poppler-pdf-bbox-layer"}


def _abbyy_marker(pages: dict[int, dict[str, Any]], page: int, key: str) -> dict[str, Any]:
    observed = JENSEITS_ABBYY_MARKER_OVERRIDES.get(key, key)
    matches = [
        record
        for record in pages[page]["records"]
        if record["compact"] == observed
    ]
    if len(matches) != 1:
        raise SourcePassageBuildError(
...
def _djvu_marker(pages: dict[int, dict[str, Any]], page: int, key: str) -> dict[str, Any]:
    ...
    return {**matches[0], "marker_basis": "exact-normalized-label-in-djvu-xml-word-layer"}
...
def _anchor_id(passage_ref: str) -> str:
    return passage_ref.replace("tos.passage.", "tos.anchor.") + ".source-passage-candidate-v1"
...
def _input_refs(repo_root: Path) -> list[dict[str, Any]]:
    paths_and_roles = [
        ...
        (ANTI_ADDRESS_ITEM_DIR / "item.manifest.json", "Antichrist-address-item-manifest"),
        (ANTI_ADDRESS_ITEM_DIR / "resource-inventory.json", "Antichrist-address-inventory"),
        (ANTI_ADDRESS_ITEM_DIR / "rights.json", "Antichrist-address-rights"),
        (ANTI_NAV_ITEM_DIR / "item.manifest.json", "Antichrist-navigation-item-manifest"),
        (ANTI_NAV_ITEM_DIR / "resource-inventory.json", "Antichrist-navigation-inventory"),
...
def _unresolved_record(
    *, candidate: dict[str, Any], expression_ref: str, passage_ref: str,
    address_witness: dict[str, Any], relation: str, boundaries: tuple[str, ...],
) -> dict[str, Any]:
    return {
...
def build_outputs(
    *, repo_root: Path, payload_source_root: Path
    ...
    if len(target_candidates) != 35:
        raise SourcePassageBuildError("target passage candidate frame must contain 35 routes")
    ...
    a_address_manifest = _read_json(repo_root / ANTI_ADDRESS_ITEM_DIR / "item.manifest.json")
    ...
    g_address = _witness(item_ref=g_manifest["item_id"], entry=g_pdf_entry, rights_ref=g_rights)
    a_address = _witness(item_ref=a_address_manifest["item_id"], entry=a_address_entry, rights_ref=a_address_rights)
    ...
    for candidate in target_candidates:
        ...
        if slug == "jenseits" and candidate["qualified_unit_key"] == "202":
            ...
            end_marker = _djvu_marker(pages, end_page, str(unit["next"]["unit_key"]))
            content_witness = _witness(
                item_ref=a_nav_manifest["item_id"], entry=a_xml_entry, rights_ref=a_nav_rights
            )
            address_offset = -2
        ...
        result = {
            ...
            "eligible_for_variant_execution": False,
            "limitations": [
                "the boundary is exact only inside the named automatic source layer",
        ...
        anchors.append(
            {
                ...
                "selector_method": {
                    "maker_type": "mixed",
                    "method": "same-source-layer numbered label to next label slice",
                    "version": "1",
    ...
    if materialized_count != 28 or unresolved_count != 7 or len(anchors) != 28:
        raise SourcePassageBuildError(
            f"expected 28 materialized / 7 unresolved / 28 anchors, got "
    ...
    output = {
        ...
        "target_passage_candidate_set_sha256": _sha256_path(repo_root / TARGET_CANDIDATE_PATH),
        "inputs": input_refs,
        "method": {
            "name": "same-source-layer-numbered-label-to-next-label-slice",
            "version": "1",
            "maker_type": "mixed",
            "layers": [
                "abbyy-xml-paragraph",
        ...
        "authority_boundary": (
            "twenty-eight private source-passage candidates are exact only "
            "inside their named automatic layers; seven boundaries remain "
    ...
    event = {
        ...
        "method": {
            ...
            "configuration": {
                ...
                "source_visible_ocr_marker_overrides": len(
        ...
        "warnings": [
            ...
            "seven source boundaries remain unresolved and have no private content",
