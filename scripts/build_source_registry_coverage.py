...
from source_registry_common import PACKET, digest, encoded, read, safe
...
def build(root: Path, *, verify_local: bool = False) -> dict:
...
def markdown(value: dict, root: Path) -> str:
    root = Path(root)
    s = value['summary']
    parent = root / PACKET

    def link(ref):
        return os.path.relpath(root / ref, parent)

    lines = ['# Покрытие реестра посадками', '', 'Проекция по исходным owner-записям, точным подготовкам и связям ветвей. Перестраивается командой `python3 -B scripts/build_source_registry_coverage.py --source-root SOURCE_ROOT --output-root OUTPUT_ROOT`.', '',
             ...
             'Это отметка о сохранённом свидетельстве приобретения. Текущее наличие и SHA-256 файлов в явно выбранном source root проверяет `python3 -B scripts/build_source_registry_coverage.py --source-root SOURCE_ROOT --verify-local`.', '',
    ...
    for t in value['targets']:
        ...
        item_label = t['edition_id']
        if t.get('status') == 'prepared_version_not_installed':
            preparation_ref = t.get('preparation_ref')
            if isinstance(preparation_ref, str) and preparation_ref:
                item_label = f"{item_label} ([подготовка]({link(preparation_ref)}), не установлено)"
            else:
                item_label = f"{item_label} (подготовлено, не установлено)"
        else:
            item_label = f"[{item_label}]({link(t['item_manifest_ref'])})"
        lines.append(f"| [{t['title']}]({link(t['work_ref'])}) | {item_label} | {t['file_count']} | {branch} |")
    lines += ['', '## Где продолжать', '', 'Только строки без подтверждённой выбранной посадки: `python3 -B scripts/build_source_registry_coverage.py --source-root SOURCE_ROOT --remaining --document A25`. Без `--document` команда выводит все такие строки JSONL. Возможные совпадения требуют проверки идентичности; они не принимаются автоматически.', '',
...
def main() -> int:
    ...
    stdout_mode = args.verify_local or args.remaining or args.document is not None
    if args.check and stdout_mode:
        parser.error('--check cannot be combined with stdout-only selectors')
    if not stdout_mode and args.output_root is None:
        parser.error('--output-root is required when writing or checking coverage outputs')
    source_root = args.source_root
    if not source_root.is_dir():
        parser.error(f'--source-root must be an existing directory: {source_root}')
    value = build(source_root, verify_local=args.verify_local)
    ...
    if args.remaining or args.document:
        ...
        return 0
    output_root = _validate_output_root(source_root, args.output_root)
    outputs = {'coverage.current.json.gz': compressed(value), 'COVERAGE.md': markdown(value, output_root).encode()}
    for name, body in outputs.items():
        path = output_root / PACKET / name
        if args.check:
