...
def output_matches(path, body):
    """Compare generated bytes while ignoring non-semantic gzip ABI drift."""
    ...
    if path.suffix != '.gz':
        return False
    try:
        return gzip.decompress(existing) == gzip.decompress(body)
...
def processor_bytes(name):
    ...
    if name == 'source_registry_common.py':
        ...
        body = body.replace('import zlib\n', '')
        start = body.index('\n\ndef compressed(value):')
        end = body.index('\ndef digest(value):', start)
        body = body[:start] + body[end:]
        start = body.index('\n\ndef output_matches(path, body):')
        end = body.index('\ndef processor_bytes(name):', start)
        body = body[:start] + body[end:]
        body = body.replace('if not output_matches(path, body):', 'if path.read_bytes() != body:')
        body = body.replace(
            'if not output_matches(snapshot / name, body):',
...
def write_immutable(path, body):
    if path.exists():
        if not output_matches(path, body):
