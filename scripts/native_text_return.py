...
from native_text_binding import NativeTextBindingError, NativeTextBindingResolver, check_local_research_rights, _json
from source_owner_context import OwnerLocalSourceContext


LOCAL_SCHEMA = 'ToS/contracts/native-local-text-read.schema.json'
PRIVATE_SCHEMA = 'ToS/contracts/native-private-text-read.schema.json'
...
class _TextReadSelection:
    ...
    def __init__(self, path, source_root):
        try:
            ...
            self.schema_path = Path(__file__).resolve().parents[1] / self.schema_ref
            self.schema_raw = _read(self.schema_path, 65_536)
            schema = _object(self.schema_raw)
            if schema.get('$id') != 'https://tree-of-sophia.local/' + self.schema_ref:
            ...
            if self.schema_ref == PRIVATE_SCHEMA:
                context = self.config['owner_context']
                context_path = _absolute(context['path'])
                if hashlib.sha256(_read(context_path, 65_536, confidential_file=True)).hexdigest() != context['sha256']:
                    raise ValueError('private reading context differs')
                self.context = OwnerLocalSourceContext.load(context_path)
                if self.context.public_root != self.source_root:
                    raise ValueError('private reading context source differs')
            self.verify()
    ...
    def verify(self):
        try:
            ...
            if self.context is not None:
                context = self.config['owner_context']
                if hashlib.sha256(_read(_absolute(context['path']), 65_536, confidential_file=True)).hexdigest() != context['sha256']:
                    raise ValueError('private reading context differs')
                self.context.snapshot()
        except (ValueError, OSError, KeyError, TypeError) as error:
            raise LocalTextReadError('local text selection is expired, revoked or changed') from error

    def _select_binding(self, resolver, binding, rights_refs=None):
        self.verify()
        ...
        if selected is None or rights_refs is not None and selected['rights_record_refs'] != rights_refs:
            raise LocalTextReadError('local text selection does not cover exact binding and rights')
        return selected


class LocalTextReadSelection(_TextReadSelection):
    """Already public-declared layers with exact conditional local notices."""

    def select(self, resolver, binding, rights_refs):
        selected = self._select_binding(resolver, binding, rights_refs)
...
class PrivateTextReadSelection(_TextReadSelection):
    """Independent protected local-research selection; never a v1 public grant.

    The configured owner context selects the private namespace. Exact binding
    admission precedes even its metadata reads. Transport access is a separate
    adapter obligation; this object is not an HTTP credential or public handle.
    """

    schema_ref = PRIVATE_SCHEMA


def read_private_unit(selection: PrivateTextReadSelection, binding: dict, *, max_return_bytes=65_536) -> dict:
    """Read one authorized native unit locally without relaxing source rights.

    No original Item payload, metadata discovery, publication, extraction or
    assessment is performed. The source's full rights records travel with the
    exact span return; they are not replaced by the selection's explanation.
    """
    if type(selection) is not PrivateTextReadSelection:
        raise LocalTextReadError('private text reading needs its separate explicit selection')
    if type(max_return_bytes) is not int or not 1 <= max_return_bytes <= 1_048_576:
        raise LocalTextReadError('private text return needs a bounded byte limit')
    selection.verify()
    binding = _json(json.dumps(binding, allow_nan=False).encode('utf-8'))
    resolver = NativeTextBindingResolver(selection.source_root, owner_context=selection.context)
    selected = selection._select_binding(resolver, binding)
    summary = resolver.resolve(binding)
    if summary['public_content_declared'] or not summary.get('owner_local_transport'):
        raise LocalTextReadError('private text selection requires the exact owner-local transport')
    layer = resolver._record(binding['text_layer']['record_ref'], expected=binding['text_layer']['record_sha256'])
    rep = layer['representation']
    selection._select_binding(resolver, binding, rep['rights_record_refs'])
    try:
        check_local_research_rights(resolver, layer)
    except PermissionError as error:
        raise LocalTextReadError('private text selection cannot override recorded rights') from error
    rights = [{**row, 'record': resolver._record(row['ref'], expected=row['sha256'])}
              for row in rep['rights_record_refs']]
    # Expiry/revocation after metadata inspection must still prevent first text I/O.
    selection.verify()
    ...
    for ref in binding['ordered_anchor_refs']:
        ...
        spans.append({'anchor_ref': ref, 'selector': dict(selector), 'exact_sha256': anchor['exact_sha256'],
                      'text': text[selector['start']:selector['end']]})
    result = {'schema_version': 'tos_native_private_unit_return_v1', 'summary': summary,
              'packet': {'id': binding['packet_id'], 'version': binding['packet_version'], 'sha256': binding['packet_sha256']},
              'layer_record_sha256': binding['text_layer']['record_sha256'],
              'representation_sha256': rep['content_sha256'], 'spans': spans,
              'rights_records': rights, 'closure_fingerprint': resolver.snapshot(),
              'local_conditions': {'selection_sha256': hashlib.sha256(selection.raw).hexdigest(),
                                   'expires_at': selection.config['expires_at'],
                                   'condition_review': selected['condition_review']},
              'authority': {'scope': 'owner-local-research', 'original_payload_returned': False,
                            'transport_scope': selection.config['transport_scope'],
                            'external_publication_authorized': False, 'assessment_applied': False}}
    selection.verify()
    if len(json.dumps(result, ensure_ascii=False, allow_nan=False, separators=(',', ':')).encode('utf-8')) > max_return_bytes:
        raise LocalTextReadError('private native unit return exceeds its output-byte budget')
    return result
