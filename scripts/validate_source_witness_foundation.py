...
def _native_topology_claims(repo_root, issues):
    ...
    from source_item_commands import verify_compound as verify_exemplar
    return [*_native_compound_claims(repo_root, issues, 'has_expression', verify_compound),
            ...
            *_native_compound_claims(repo_root, issues, 'exemplified_by', verify_exemplar)]
...
def _validate_composite_representation_payload(
    ...
    *,
    ...
    """Validate one scholarly-composite representation and return a safe path."""
    ...
    return _composite_payload_path(
...
def _composite_payload_path(
    ...
    """Resolve one direct, non-symlink file in the owned payload directory."""
    if not isinstance(payload, dict):
        return None
    ...
    try:
        ...
        if payload_path.resolve() != expected_path:
            return None
        if payload_path.exists() and not payload_path.is_file():
            return None
...
def validate_composite_payload_file(
    ...
    *,
    ...
    """Check local composite custody independently of the source declaration."""
    issues: list[Issue] = []
    ...
    if payload_path is None:
        return [
            (
                location,
                "scholarly-composite payload must be a direct local file under its "
                "representation payload/ without symlinks",
            )
        ]
    ...
    if has_git:
        for metadata_path in sorted(metadata_paths):
            if _git_tracked(repo_root, metadata_path) is not True:
                issues.append(
                    (
                        _relative(metadata_path, repo_root),
    ...
    if (
        ...
        issues.append(
            (
                location,
                "scholarly-composite payload must declare either "
                "not_materialized/unmaterialized_payload or "
                "materialized/local_gitignored_payload, with git_tracked=false",
            )
        )
    ...
    if tracked is True:
        issues.append(
            (location, "local scholarly-composite payload must not be Git-tracked")
        )
    ...
    if ignored is False:
        issues.append(
            (location, "local scholarly-composite payload is not ignored by Git")
        )
    elif ignored is None and has_git:
        issues.append(
            (location, "could not determine scholarly-composite payload ignore posture")
        )

    if representation.get("file_id") != f"tos.file.sha256.{payload.get('sha256')}":
        issues.append(
            (location, "scholarly-composite representation file_id is not content-addressed")
        )
    ...
    if declared_status != "materialized":
        issues.append(
            (
                location,
                "present scholarly-composite payload must declare "
                "materialized/local_gitignored_payload",
            )
        )
    if payload_path.stat().st_size != payload.get("byte_size"):
        issues.append(
            (location, "scholarly-composite representation byte_size differs from payload")
        )
    if _sha256(payload_path) != payload.get("sha256"):
        issues.append(
            (location, "scholarly-composite representation sha256 differs from payload")
        )
    return issues
...
def _validate_foundation(
    ...
    *,
    ...
    for representation_path in sorted(composite_root.glob("**/representations/*/representation.json")):
        ...
        if payload_path is None:
            # A schema-invalid, unsafe, or non-regular path cannot safely
            # participate in downstream rights or provenance reads.
            continue
